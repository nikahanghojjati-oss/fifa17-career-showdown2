...
6. Prefer deterministic contracts, two-context browser automation and Auth+Firestore-emulator proof before owner effort. Build a production acceptance path only for aspects that cannot be automated, especially genuine two-physical-device/two-network behavior.
7. Require every current permanent workflow family green on the same exact reviewed PR head, specifically the current PR #192 head. A newer commit invalidates older exact-head evidence. Resolve all meaningful review findings, merge only with expected-head protection, then independently verify post-merge main, Pages deployment and production bytes/runtime provenance.
8. After the two-network/reconnect boundary is genuinely closed, execute final stable Remote Joining release acceptance as the remaining explicit lane toward genuine RJR100.
...
<!-- Historical compatibility provenance only; NOT current execution authority. CURRENT OVERRIDE — PR #171 MERGED — RJR87 — STAGE 5A. STAGE 5A IS AUTHORIZED NEXT at that historical checkpoint; runtime implementation has not started. -->