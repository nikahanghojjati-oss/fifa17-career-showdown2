...
If a completed product step is not reflected promptly in the panel, press `CHECK NOW`; do not repeat a provider mutation merely to make the recorder notice it. Keep the same manager roles, accounts, registered devices and rivalry for the full attempt. If the panel ever shows `AUTHORITY` incomplete, discard that attempt and use `RESET RECORDER` before starting a fresh run; returning to the earlier identity does not clear the sticky authority violation.
...
3. Only after Shared History has converged, take each device genuinely offline once and then reconnect it. Keep it offline long enough to make the browser genuinely observe the offline state, then reconnect. Wait until the Physical Journey panel shows OFFLINE RECOVERY complete before continuing. The recorder performs the replaceable conflict-guard proof automatically; do not manufacture a conflict manually.
...
8. Confirm the panel still shows the saved device/network labels, `AUTHORITY` complete, and the expected completed journey indicators. On each device, press `DOWNLOAD JSON` (or `COPY EVIDENCE` if download is unavailable) and provide the two sanitized exports together.
