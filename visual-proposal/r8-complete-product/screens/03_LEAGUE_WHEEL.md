...
Status: ACTIVE r18 SCREEN CONTRACT — R8.43 FINAL MINOR POLISH BROWSER-PROVEN / OWNER REVIEW OPEN / PRODUCTION INTEGRATION NOT AUTHORIZED
...
The exact R8.43 owner-review snapshot is persisted in the Showdown visual Library at `/Showdown visual/R8_43_League/league-r8-43-final-polish.html`, with ready/selected Chromium screenshots and the full browser QA report beside it.

Current evidence:

- `evidence/LEAGUE_WHEEL_R8_42_REFERENCE_GEOMETRY_LIGHTING_BROWSER_PROOF_2026-09-11.md`
- `evidence/LEAGUE_R8_43_FINAL_POLISH_HOME_R8_44_BROWSER_BUILD_PROOF_2026-09-11.md`
...
R8.43 keeps the existing `#topHeader` authority and only restyles it as a lean translucent cinematic header carrying CM17 identity, screen context and `#seasonIndicator`.
...
- gold rough/brush-directed `SELECT LEAGUE` DOM title with physical header/light clearance;
...
R8.43 wide-desktop owner-review sources remain:
...
- Nik: A01 core thinking hero, positioned/rescaled against the reference geometry.

Daniel's earlier synthetic CSS edge glow remains removed. His hand approaches the outer decorative rim with controlled contact rather than covering a large wheel area. Character pixels never carry wheel state, hit targets or selection authority.

Characters remain `aria-hidden`, pointer-inert and unnecessary for operating the screen.

## Geometry authority

R8.42 established the measurable manager/wheel composition against the owner reference. At 1366×768 the approximate composition checks were:

- Daniel target x≈233, y≈175, width≈138; browser x≈238, y≈177, width≈131;
- Nik target x≈1030, y≈155, width≈158; browser x≈1032, y≈157, width≈153.

These measurements are composition-only checks, not biometric identity authority.

R8.43 preserves that geometry and performs only bounded mark/name/title polish.
...
These are original proposal identities, not reproductions of official competition trademarks and not crops from a generated logo-board screenshot.

Reference composition order remains:
...
### R8.43 mark/name polish

All five segments now use one deterministic internal mark/name grid. SVG marks are constrained to a common visual box and league-name rows use a common height/baseline, preventing mark/name collision or inconsistent vertical gaps. Browser QA explicitly checks every mark/name pair for overlap.
...
R8.43 gives the title a dedicated clean visual zone and higher presentation z-order so stadium/floodlight treatment cannot wash over or obscure the top of the gold lettering. Typography remains owner-reviewable; no font treatment is owner-final until explicitly approved.
...
`#selectedLeague` and `#leagueStateNote` remain in the DOM for runtime/accessibility compatibility even when visible status is presented elsewhere in the composition.
...
R8.43 full browser matrix: six states × six viewport conditions = **36/36 PASS**.
...
Checks include no horizontal overflow, no title/header overlap, no wheel/action overlap, exactly five wheel items, no mark/name overlap, fallback character removal and expected disabled-state presentation.

## Image-generation execution rule

Whole-screen Select League generations remain `CONCEPT_ONLY / REJECTED_AS_IMPLEMENTATION`.

R8.43 used no new image-generation call. The browser render is the owner-review surface. The image-generation hard gate remains locked.
