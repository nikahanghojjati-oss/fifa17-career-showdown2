...
First read `SESSION_BOOTSTRAP.json` and `00_SESSION_BOOTSTRAP.md` from the repository, then use the connected GitHub source to verify current live `main`, latest relevant merged/open PRs, current candidate CI if any, runtime identity, and `WORK_ENVIRONMENT_STATUS.json`.
