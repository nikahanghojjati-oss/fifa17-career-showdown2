...
const remote='<button class="menuButton" id="remoteJoiningButton" type="button">PRIVATE REMOTE JOINING</button>';
...
const shell='    document.getElementById("remoteJoiningButton").onclick=()=>Promise.all([loadRuntimeStyle("rj","css/remoteJoining.css"),loadRuntimeScript("rj","js/sparkRemoteJoining.js",()=>window.CareerModeSparkRemoteJoining)]).then(()=>window.CareerModeSparkRemoteJoining.openPanel()).catch(console.error);\n';
...
contract=contract.replace('assert.ok(app.includes(\'id="remoteJoiningButton"\')||app.includes(\'button.id="remoteJoiningButton"\'));\nassert.ok(app.includes("loadRemoteJoiningSurface"));','assert.ok(html.includes(\'id="remoteJoiningButton"\'));\nassert.ok(optional.includes("remoteJoiningButton")&&optional.includes("js/sparkRemoteJoining.js")&&optional.includes("css/remoteJoining.css"));');
...
console.log(`STAGE5E_STARTUP_BUDGET raw=${raw}/165000 gzip=${gz}/37500 binding=${Buffer.byteLength(shell)}`);
if(raw>165000||gz>37500)throw new Error(`Stage 5E static launcher still exceeds startup budget: ${raw}/${gz}`);
