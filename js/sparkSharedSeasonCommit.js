(function(root,factory){
  ...
  function scpAssertRivalry(value,rivalryId,uid){
    ...
    if(ordered.some(slot=>!slot||slot.entitlementState!=="active"||typeof slot.accountId!=="string"||!/^profile_[0-9a-f]{24}$/.test(String(slot.profileId||""))||!/^save_[0-9a-f]{24}$/.test(String(slot.saveId||"")))||ordered[0].accountId===ordered[1].accountId||!ordered.every(slot=>authorized.includes(slot.accountId)))scpFail("SEASON_COMMIT_BINDING_INVALID");
  ...
  function scpAssertSession(value,rivalryId,sessionId,authorized,now){const members=value?.data?.memberAccountIds,expires=scpTimestampMillis(value?.data?.expiresAt),host=value?.data?.hostAccountId;if(!value||value.objectType!=="session"||value.objectId!==sessionId||value.lifecycleState!=="live"||value.data?.rivalryId!==rivalryId||value.data?.state!=="active"||!Array.isArray(members)||members.length!==2||new Set(members).size!==2||!authorized.every(id=>members.includes(id))||!members.every(id=>authorized.includes(id))||!authorized.includes(host)||!Number.isFinite(expires)||now>=expires)scpFail("SEASON_COMMIT_ACTIVE_SESSION_REQUIRED");return Object.freeze({hostAccountId:host});}
  ...
  function scpAuthority({rivalryId,rivalry,role,sessionId,deviceId,hostAccountId}){const slot=rivalry.slots.find(item=>item.slotId===role),host=rivalry.slots.find(item=>item.accountId===hostAccountId);if(!slot||!host)scpFail("SEASON_COMMIT_SETUP_NOT_CONFIRMED");return {rivalryId,connectionState:"active",managerSlots:rivalry.slots.map(item=>({slotId:item.slotId,accountId:item.accountId,profileId:item.profileId,saveId:item.saveId,accountState:"active",entitlementState:"active"})),actor:{accountId:slot.accountId,deviceId,deviceState:"active",managerRole:role,profileId:slot.profileId,saveId:slot.saveId},session:{sessionId,rivalryId,state:"active",hostAccountId,memberAccountIds:[...rivalry.authorized],expiresAtEpochMs:Number.MAX_SAFE_INTEGER},nowEpochMs:0};}
  async function scpRebuildSetup(ledger,rivalry,rivalryId,sessionId,deviceId,hostAccountId,cryptoImpl){
  ...
  async function scpCoreFromStorage(value,ready,teamCount,cryptoImpl){
    ...
    scpExact(value.results,ROLES,"SEASON_COMMIT_PROVIDER_STATE_INVALID");for(const role of ROLES)scpResult(value.results[role],teamCount);
  ...
  async function scpContext(tx,options){
    ...
    scpAssertAccount(await scpGet(tx,accountRef),uid);scpAssertDevice(await scpGet(tx,deviceRef),deviceId);const rivalry=scpAssertRivalry(await scpGet(tx,rivalryRef),rivalryId,uid),session=scpAssertSession(await scpGet(tx,sessionRef),rivalryId,sessionId,rivalry.authorized,now),ledger=scpAssertSetup(await scpGet(tx,setupRef),rivalryId,seasonNumber),cryptoImpl=options.cryptoImpl||root.crypto,setup=await scpRebuildSetup(ledger,rivalry,rivalryId,sessionId,deviceId,session.hostAccountId,cryptoImpl),teamCount=scpTeamCount(setup);const publicResult=scpAssertPublicResults(await scpGet(tx,publicRef),rivalryId,seasonNumber);const p1=scpAssertPrivateResult(await scpGet(tx,p1Ref),rivalryId,seasonNumber,"playerOne",teamCount),p2=scpAssertPrivateResult(await scpGet(tx,p2Ref),rivalryId,seasonNumber,"playerTwo",teamCount);const ready=await scpReadyState(publicResult,{playerOne:p1,playerTwo:p2},teamCount,cryptoImpl);const stored=await scpGet(tx,commitRef);
