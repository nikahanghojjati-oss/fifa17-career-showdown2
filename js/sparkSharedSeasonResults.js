(function(root,factory){
  ...
  const PUBLIC_KEYS=Object.freeze(["schemaVersion","objectType","rivalryId","seasonNumber","runtimeRevision","phase","revision","publishedRoles","operationIds","operationHashes","baseRevisions","actorRoles","activeSessionId","updatedAt","updatedByDeviceId"]);
  ...
  async function ssrpContext(options,transaction,{readOpponent=false}={}){
    ...
    const transfer=ssrpAssertTransfer(ssrpSnapshot(await transaction.get(refs.transfer)),rivalryId,seasonNumber,ledger),state=ssrpPublicState(ssrpSnapshot(await transaction.get(refs.public)),rivalryId,seasonNumber);await ssrpVerifyPublicHashes(state,cryptoImpl);
