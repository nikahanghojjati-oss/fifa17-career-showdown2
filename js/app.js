...
{const m=document.createElement("link");m.rel="manifest";m.href="manifest.webmanifest?v=1.1.5-r1";document.head.append(m);}
...
b.addEventListener("click",()=>{if(runtimeNoticeTimer)window.clearTimeout(runtimeNoticeTimer);runtimeNoticeTimer=null;n.remove();runtimeNoticeElement=runtimeNoticeTextElement=null;});
...
const n=getRuntimeNotice();if(n.className!==type)n.className=type;
const m=String(message||"");if(runtimeNoticeTextElement&&runtimeNoticeTextElement.textContent!==m)runtimeNoticeTextElement.textContent=m;
if(runtimeNoticeTimer)window.clearTimeout(runtimeNoticeTimer);
runtimeNoticeTimer=duration>0?window.setTimeout(()=>{if(runtimeNoticeElement?.isConnected)runtimeNoticeElement.remove();runtimeNoticeElement=runtimeNoticeTextElement=runtimeNoticeTimer=null;},duration):null;
}
...
requestAnimationFrame(()=>{revealApplication();const s=document.createElement("script");s.src="js/offlineApp.js?v=1.1.5-r1";document.body.append(s);scheduleApplicationDiagnostics();});
}
document.readyState==="loading"?document.addEventListener("DOMContentLoaded",startApplication,{once:true}):startApplication();
