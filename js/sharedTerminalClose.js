(function(root,factory){
  const finalModule=typeof module!=="undefined"&&module.exports
    ?require("./sharedFinalReconciliation.js")
    :root.CareerModeSharedFinalReconciliation;
  const api=factory(finalModule);
  if(typeof module!=="undefined"&&module.exports)module.exports=api;
  else root.CareerModeSharedTerminalClose=api;
})(typeof globalThis!=="undefined"?globalThis:this,function(finalModule){
  "use strict";

  const RUNTIME_REVISION="1.9.1-r18";
  const READY_PHASE="TERMINAL_CLOSE_READY";
  const CLOSED_PHASE="TERMINAL_CLOSED";
  const ROLES=Object.freeze(["playerOne","playerTwo"]);
  const INTENT_KEYS=Object.freeze([
    "schemaVersion","runtimeRevision","phase","rivalryId","acceptedRevisionKey","totalSeasons","completedSeason",
  ...
  function tcPrepare(finalReconciliation){
    if(!finalModule||typeof finalModule.verifyProjection!=="function")tcFail("TERMINAL_CLOSE_FINAL_PROTOCOL_UNAVAILABLE");
    let final;
    try{final=finalModule.verifyProjection(finalReconciliation);}catch(_error){tcFail("TERMINAL_CLOSE_FINAL_RECONCILIATION_REQUIRED");}
    return tcVerifyIntent({
      schemaVersion:1,
      runtimeRevision:RUNTIME_REVISION,
      phase:READY_PHASE,
      rivalryId:final.rivalryId,
      acceptedRevisionKey:final.acceptedRevisionKey,
      totalSeasons:final.totalSeasons,
      completedSeason:final.completedSeason,
      fixedClubs:tcClone(final.fixedClubs),
      managerTotals:tcClone(final.managerTotals),
      winner:final.winner,
      terminal:true,
      finalSeasonReconciled:true,
      nextSeason:null,
      extraSeasonAllowed:false,
      rivalryConnectionState:"closed",
      sessionTargetState:"closed",
      terminalReadAllowed:true,
  ...
  function tcClosed(intent,providerResult){
    ...
    if(String(providerResult.rivalryId||"")!==verified.rivalryId||String(providerResult.sessionState||"")!=="closed"||String(providerResult.rivalryState||"")!=="closed")tcFail("TERMINAL_CLOSE_PROVIDER_RESULT_INVALID");
    return tcFreeze({
      schemaVersion:1,
      runtimeRevision:RUNTIME_REVISION,
      phase:CLOSED_PHASE,
      rivalryId:verified.rivalryId,
      acceptedRevisionKey:verified.acceptedRevisionKey,
      totalSeasons:verified.totalSeasons,
      winner:verified.winner,
      managerTotals:tcClone(verified.managerTotals),
      rivalryRevision:providerResult.rivalryRevision,
      sessionRevision:providerResult.sessionRevision,
      replayed:providerResult.replayed===true,
      terminal:true,
      nextSeason:null,
      extraSeasonAllowed:false,
      canonicalStorageMutation:false,
  ...
  return Object.freeze({
    contractVersion:1,
    feature:"ssjr-shared-terminal-close",
    runtimeRevision:RUNTIME_REVISION,
    readyPhase:READY_PHASE,
    closedPhase:CLOSED_PHASE,
    prepare:tcPrepare,
    verifyIntent:tcVerifyIntent,
    witnessKey:tcWitnessKey,
    sameWitness:tcSameWitness,
    closeResult:tcClosed,
    createsAdditionalSeason:false,
    preservesAccumulatedScoring:true,
    terminalReadAllowed:true,
    canonicalStorageMutation:false,
    providerWriteRequired:true,
    listPermissionRequired:false,
