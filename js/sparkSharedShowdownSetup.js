(function(root,factory){
  ...
  function ssrpExact(value,keys){return !!value&&typeof value==="object"&&!Array.isArray(value)&&Object.keys(value).length===keys.length&&keys.every(key=>Object.hasOwn(value,key));}
  function ssrpNormalizeRivalryId(value){const id=String(value||"").trim().toLowerCase();if(!/^pair_[0-9a-f]{64}$/.test(id))ssrpFail("SETUP_RIVALRY_INVALID");return id;}
  function ssrpNormalizeSessionId(value){const id=String(value||"").trim().toLowerCase();if(!/^session_[0-9a-f]{64}$/.test(id))ssrpFail("SETUP_SESSION_INVALID");return id;}
  function ssrpNormalizeDeviceId(value){const id=String(value||"").trim().toLowerCase();if(!/^device_[0-9a-f]{32}$/.test(id))ssrpFail("SETUP_DEVICE_INACTIVE");return id;}
  function normalizeOperationId(value){const id=String(value||"").trim().toLowerCase();if(!/^setup_op_[0-9a-f]{32}$/.test(id))ssrpFail("SETUP_COMMAND_INVALID");return id;}
  function accountId(user){const id=user&&typeof user.uid==="string"?user.uid.trim():"";if(!id)ssrpFail("SETUP_AUTH_REQUIRED");return id;}
  ...
  function nowEpochMs(value){const result=Number(value===undefined?Date.now():value);if(!Number.isSafeInteger(result)||result<0)ssrpFail("SETUP_CLOCK_INVALID");return result;}
  function ssrpValidateSdk(firestore,sdk){
    if(!firestore)ssrpFail("SETUP_PROVIDER_UNAVAILABLE");
    for(const name of ["doc","runTransaction"]){if(!sdk||typeof sdk[name]!=="function")ssrpFail("SETUP_PROVIDER_UNAVAILABLE");}
  ...
  function assertAccount(value,id){
    if(!value||value.objectType!=="account"||value.objectId!==id||value.lifecycleState!=="live"||!value.data||value.data.status!=="active")ssrpFail("SETUP_MANAGER_INACTIVE");
    return value;
  }
  function assertDevice(value,id){
    if(!value||value.objectType!=="device"||value.objectId!==id||value.lifecycleState!=="live"||!value.data||value.data.deviceId!==id||value.data.state!=="active")ssrpFail("SETUP_DEVICE_INACTIVE");
    return value;
  }
  function assertRivalry(value,rivalryId,uid){
    if(!value||value.objectType!=="rivalry"||value.objectId!==rivalryId||value.lifecycleState!=="live"||!value.data||value.data.connectionState!=="active")ssrpFail("SETUP_RIVALRY_INACTIVE");
    ...
    if(slots.length!==2||authorized.length!==2||new Set(authorized).size!==2)ssrpFail("SETUP_TWO_MANAGERS_REQUIRED");
    const ordered=ROLES.map(role=>slots.find(item=>item&&item.slotId===role));
    if(ordered.some(item=>!item||item.entitlementState!=="active"||typeof item.accountId!=="string"||!/^profile_[0-9a-f]{24}$/.test(item.profileId||"")||!/^save_[0-9a-f]{24}$/.test(item.saveId||"")))ssrpFail("SETUP_BINDING_INVALID");
    if(ordered[0].accountId===ordered[1].accountId||!ordered.every(item=>authorized.includes(item.accountId))||!authorized.every(id=>ordered.some(item=>item.accountId===id)))ssrpFail("SETUP_TWO_MANAGERS_REQUIRED");
    const actor=ordered.find(item=>item.accountId===uid);
    if(!actor)ssrpFail("SETUP_ACTOR_NOT_ENTITLED");
    return Object.freeze({slots:ordered,authorized,actor});
  }
  function assertSession(value,rivalryId,sessionId,authorized,epochMs){
    if(!value||value.objectType!=="session"||value.objectId!==sessionId||value.lifecycleState!=="live"||!value.data)ssrpFail("SETUP_SESSION_INVALID");
  ...
  function assertLedger(value,rivalryId){
    if(!ssrpExact(value,LEDGER_KEYS)||value.schemaVersion!==1||value.objectType!=="sharedSetupLedger"||value.rivalryId!==rivalryId||!Number.isInteger(value.revision)||value.revision<1||value.revision>6||!ROLES.includes(value.coordinatorRole)||value.phase!==expectedPhase(value.revision))ssrpFail("SETUP_PROVIDER_STATE_INVALID");
    for(const key of ["operationIds","operationTypes","baseRevisions","actorRoles"]){if(!Array.isArray(value[key])||value[key].length!==value.revision)ssrpFail("SETUP_PROVIDER_STATE_INVALID");}
    if(new Set(value.operationIds).size!==value.operationIds.length||value.operationIds.some(id=>!/^setup_op_[0-9a-f]{32}$/.test(id)))ssrpFail("SETUP_PROVIDER_STATE_INVALID");
    if(value.operationTypes.some((type,index)=>type!==TYPES[index])||value.baseRevisions.some((base,index)=>base!==index)||value.actorRoles.some(role=>!ROLES.includes(role)))ssrpFail("SETUP_PROVIDER_STATE_INVALID");
    if(value.actorRoles.slice(0,4).some(role=>role!==value.coordinatorRole))ssrpFail("SETUP_PROVIDER_STATE_INVALID");
    if(value.revision<4?value.totalSeasons!==null:!LENGTHS.includes(value.totalSeasons))ssrpFail("SETUP_PROVIDER_STATE_INVALID");
    const confirmations=value.revision<=4?[]:value.actorRoles.slice(4);
    if(!Array.isArray(value.confirmedRoles)||JSON.stringify(value.confirmedRoles)!==JSON.stringify(confirmations)||new Set(confirmations).size!==confirmations.length)ssrpFail("SETUP_PROVIDER_STATE_INVALID");
    if(!/^session_[0-9a-f]{64}$/.test(value.activeSessionId||"")||!/^device_[0-9a-f]{32}$/.test(value.updatedByDeviceId||""))ssrpFail("SETUP_PROVIDER_STATE_INVALID");
    return value;
  }
  ...
  function authority({rivalryId,rivalry,session,role,deviceId,epochMs,hostRole}){
    ...
    if(!slot||!host)ssrpFail("SETUP_SESSION_MEMBERS_MISMATCH");
    return {
      rivalryId,
  ...
  function syntheticSession(rivalryId,rivalry,coordinatorRole,sessionId){
    ...
    if(!host)ssrpFail("SETUP_PROVIDER_STATE_INVALID");
  ...
  async function rebuild(protocol,ledger,rivalry,rivalryId,cryptoImpl){
    ...
    for(let index=0;index<ledger.revision;index++){
      ...
      if(!applied.ok)ssrpFail("SETUP_PROVIDER_STATE_INVALID");
      state=applied.state;
    }
    return state;
  }
  ...
  async function context(options,transaction){
    ...
    const rivalryId=ssrpNormalizeRivalryId(options.rivalryId);
    const sessionId=ssrpNormalizeSessionId(options.sessionId);
    const deviceId=ssrpNormalizeDeviceId(options.deviceId);
    const epochMs=nowEpochMs(options.nowEpochMs);
    ssrpValidateSdk(options.firestore,options.firebaseSdk);
    ...
    assertAccount(ssrpSnapshot(await transaction.get(refs.account)),uid);
    assertDevice(ssrpSnapshot(await transaction.get(refs.device)),deviceId);
    const rivalry=assertRivalry(ssrpSnapshot(await transaction.get(refs.rivalry)),rivalryId,uid);
    const session=assertSession(ssrpSnapshot(await transaction.get(refs.session)),rivalryId,sessionId,rivalry.authorized,epochMs);
    const setupValue=ssrpSnapshot(await transaction.get(refs.setup));
  ...
  async function mutate(options={}){
    try{
      ...
      if(!["open","commit-league","commit-clubs","commit-length","confirm"].includes(type))ssrpFail("SETUP_COMMAND_INVALID");
      const baseRevision=Number(options.baseRevision);
      if(!Number.isInteger(baseRevision)||baseRevision<0)ssrpFail("SETUP_COMMAND_INVALID");
      if(type==="commit-length"&&!LENGTHS.includes(options.totalSeasons))ssrpFail("SETUP_LENGTH_INVALID");
      ssrpValidateSdk(options.firestore,options.firebaseSdk);
      ...
      return await options.firebaseSdk.runTransaction(options.firestore,async transaction=>{
        ...
        if(!applied.ok)ssrpFail(applied.code);
