(function(root,factory){
  ...
  let ssjrNegProbeLastActive=null;
  function ssjrNegProbeStorageSnapshot(){
    if(typeof root.captureCareerModeRawBackupInputs!=="function")ssjrNegProbeFail("SSJR_NEGATIVE_STORAGE_AUTHORITY_UNAVAILABLE");
    const raw=root.captureCareerModeRawBackupInputs();
    const keys=["saveLibrary","legacyShowdowns","preferences"];
    if(!raw||keys.some(key=>!Object.hasOwn(raw,key)))ssjrNegProbeFail("SSJR_NEGATIVE_STORAGE_AUTHORITY_UNAVAILABLE");
    return JSON.stringify(keys.map(key=>[key,raw[key]]));
  }
  function ssjrNegProbeStorageView(){const keys=["saveLibrary","legacyShowdowns","preferences"];return Object.freeze({length:keys.length,key:index=>keys[index],getItem:key=>JSON.parse(ssjrNegProbeStorageSnapshot()).find(row=>row[0]===key)?.[1]??null});}
  async function ssjrNegProbeIdentity(ctx){
    if(ctx.services.auth.currentUser?.uid!==ctx.user.uid)ssjrNegProbeFail("SSJR_NEGATIVE_IDENTITY_CHANGED");
    const hash=async value=>{const bytes=await root.crypto.subtle.digest("SHA-256",new TextEncoder().encode(value));return `sha256:${Array.from(new Uint8Array(bytes),v=>v.toString(16).padStart(2,"0")).join("")}`;};
    const identity={managerRole:ctx.state.managerRole,accountFingerprint:await hash(ctx.user.uid),deviceFingerprint:await hash(ctx.state.deviceId),rivalryFingerprint:await hash(ctx.state.rivalryId)};
    const current=ctx.setup.getState();
    if(ctx.services.auth.currentUser?.uid!==ctx.user.uid||current?.ready===true&&["accountId","deviceId","rivalryId","managerRole"].some(key=>current[key]!==ctx.state[key]))ssjrNegProbeFail("SSJR_NEGATIVE_IDENTITY_CHANGED");
    return identity;
  }
  ...
  async function ssjrNegProbeBaseContext(allowExpired=false){
    ...
    const current=setup.getState();
    const state=current&&current.ready===true?{...current}:allowExpired&&ssjrNegProbeLastActive;
    if(!state||!state.rivalryId||!state.sessionId||!state.deviceId||!state.managerRole)ssjrNegProbeFail("SSJR_NEGATIVE_ACTIVE_SETUP_REQUIRED","Resolve the exact ACTIVE Shared Setup authority before running this probe.");
    ...
    if(state.accountId!==user.uid)ssjrNegProbeFail("SSJR_NEGATIVE_IDENTITY_CHANGED");
    if(current&&current.ready===true)ssjrNegProbeLastActive=state;
  ...
  async function ssjrNegProbeFinalize(name,code,before,ctx){if(!EXPECTED[name]||!EXPECTED[name].has(code))ssjrNegProbeFail("SSJR_NEGATIVE_UNEXPECTED_DENIAL",`${name} returned an unexpected result.`);const identity=await ssjrNegProbeIdentity(ctx);if(before!==ssjrNegProbeStorageSnapshot())ssjrNegProbeFail("SSJR_NEGATIVE_LOCAL_STORAGE_CHANGED");return Object.freeze({ok:true,probeContract:CONTRACT,denied:true,source:SOURCES[name],code,localStorageUnchanged:true,...identity});}
  async function ssjrNegProbeRunInternal(name){
    const ctx=await ssjrNegProbeBaseContext(name==="expiredSession"),before=ssjrNegProbeStorageSnapshot();let code=null;
    if(name==="wrongSession"){const result=await ctx.adapter.read(ssjrNegProbeAdapterOptions(ctx,{sessionId:ssjrNegProbeAlternateSessionId(ctx.state.sessionId)}));code=result&&result.ok===false?result.code:null;}
    else if(name==="expiredSession"){const auth=await ssjrNegProbeReadAuthority(ctx);if(Date.now()<auth.authority.session.expiresAtEpochMs)ssjrNegProbeFail("SSJR_NEGATIVE_SESSION_NOT_EXPIRED","This session has not expired in real time. Keep this page open and retry after expiry.");const result=await ctx.adapter.read(ssjrNegProbeAdapterOptions(ctx));code=result&&result.ok===false?result.code:null;}
      ...
      if(name==="directFieldSubstitution"){
        if(auth.actorRole!==current.state.coordinatorRole)ssjrNegProbeFail("SSJR_NEGATIVE_PROBE_ROLE_NOT_SUITABLE","Direct draw substitution must be tested by the coordinator before the draw.");
    ...
    return ssjrNegProbeFinalize(name,code,before,ctx);
  }
  async function ssjrNegProbeRunStage5f(name,options={}){
    ...
    if(name==="revokedIdentity"){
      ...
      if(actor?.deviceId!==ctx.state.deviceId)ssjrNegProbeFail("SSJR_NEGATIVE_IDENTITY_CHANGED");
      result=await stage.probeRevokedDeviceProviderDenial({user:ctx.user,firestore:ctx.services.firestore,firebaseSdk:ctx.services.firestoreSdk,pairingApi:pairing,protocolApi:protocol,actorIdentity:actor,rivalryId:ctx.state.rivalryId,localStorageImpl:ssjrNegProbeStorageView(),indexedDBImpl:root.indexedDB,cryptoImpl:root.crypto});
    }else if(name==="unrelatedAccount"){
      ssjrNegProbeFail("SSJR_NEGATIVE_ACCOUNT_CONTEXT_CONFLICT","An unrelated account cannot also be this rivalry's bound manager. A separate actor evidence contract is required.");
    }else ssjrNegProbeFail("SSJR_NEGATIVE_NAME_INVALID");
    ...
    return ssjrNegProbeFinalize(name,result.code,before,ctx);
  }
  async function ssjrNegProbeRun(name,options={}){if(name==="unrelatedAccount")ssjrNegProbeFail("SSJR_NEGATIVE_ACCOUNT_CONTEXT_CONFLICT","An unrelated account cannot also be this rivalry's bound manager. A separate actor evidence contract is required.");if(name==="revokedIdentity")return ssjrNegProbeRunStage5f(name,options);return ssjrNegProbeRunInternal(name);}
  function ssjrNegProbeReset(){ssjrNegProbeLastActive=null;}
  return Object.freeze({contractVersion:1,feature:FEATURE,probeContract:CONTRACT,sources:SOURCES,run:ssjrNegProbeRun,reset:ssjrNegProbeReset,billingRequired:false,blazeRequired:false,cloudRunRequired:false,cloudFunctionsRequired:false,canonicalStorageMutation:false});
});
