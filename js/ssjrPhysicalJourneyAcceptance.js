(function(root,factory){
  ...
  function template(){return {schemaVersion:1,runtimeRevision:revision(),appVersion:appVersion(),startedAt:now(),startupCount:0,deviceLabel:"",networkLabel:"",managerRole:null,remoteRole:null,accountFingerprint:null,deviceFingerprint:null,rivalryFingerprint:null,sessionFingerprints:[],authorityViolation:false,canonicalStorageProofScope:"local-reconciliation-preview",canonicalStorageBeforeHash:null,canonicalStorageAfterHash:null,canonicalStorageViolation:false,candidateCApplied:false,offlineObserved:false,onlineRecovered:false,reconnectRecoveredStartupCount:null,reloadResumed:false,terminalReloadVerified:false,conflictGuardProven:false,terminalClosedStartupCount:null,milestones:[],sequence:0};}
  ...
  function authorityError(message){safe.authorityViolation=true;persist();throw new Error(message);}
  ...
  async function captureLocalReconciliationBaseline(){
    if(!enabled)return false;const hash=await canonicalHash();
    if(!hash){safe.canonicalStorageViolation=true;persist();return false;}
    if(safe.canonicalStorageBeforeHash&&safe.canonicalStorageBeforeHash!==hash){safe.canonicalStorageViolation=true;persist();return false;}
    safe.canonicalStorageBeforeHash=hash;safe.canonicalStorageAfterHash=null;milestone("local-reconciliation-storage-baseline","CAPTURED",{providerWrite:false});persist();return true;
  }
  async function verifyLocalReconciliationPreview(){
    if(!enabled)return false;const hash=await canonicalHash();safe.canonicalStorageAfterHash=hash;
    if(!safe.canonicalStorageBeforeHash||!hash||safe.canonicalStorageBeforeHash!==hash)safe.canonicalStorageViolation=true;
    milestone("local-reconciliation-storage-verified",safe.canonicalStorageViolation?"CHANGED":"UNCHANGED",{providerWrite:false});persist();return safe.canonicalStorageViolation!==true;
  }
  ...
  function hasStage(stage){return safe.milestones.some(item=>item.stage===stage);}
  ...
  async function bindIdentity(){
    ...
    if(managerRole&&managerRole!==safe.managerRole){if(safe.managerRole)authorityError("Manager role changed during physical acceptance.");safe.managerRole=managerRole;}
    if(remoteRole&&remoteRole!==safe.remoteRole){if(safe.remoteRole)authorityError("Remote role changed during physical acceptance.");safe.remoteRole=remoteRole;}
    for(const [field,raw] of [["accountFingerprint",accountId],["deviceFingerprint",deviceId],["rivalryFingerprint",rivalryId]])if(raw){const fingerprint=await sha256(raw);if(safe[field]&&safe[field]!==fingerprint)authorityError(`${field} changed during physical acceptance.`);safe[field]=fingerprint;}
  ...
  function observeMilestones(identity){
    ...
    if(terminal?.phase==="CLOSED"&&terminal.terminal===true){
      ...
      if(safe.startupCount>safe.terminalClosedStartupCount){safe.terminalReloadVerified=true;milestone("terminal-reload-verified","CLOSED_AFTER_RELOAD",{startupCount:safe.startupCount});}
      persist();
    }
  }
  ...
  function completed(){const stages=new Set(safe.milestones.map(item=>item.stage)),previewReady=safe.milestones.some(item=>item.stage==="local-reconciliation-safe"&&item.phase==="PREVIEW_READY");return CORE_STAGES.every(stage=>stages.has(stage))&&previewReady&&stages.has("conflict-guard-proven")&&safe.offlineObserved&&safe.onlineRecovered&&safe.reloadResumed&&safe.terminalReloadVerified&&safe.conflictGuardProven&&safe.authorityViolation!==true&&safe.candidateCApplied===false&&storageProofComplete();}
  async function evidence(){await queue;await observe();persist();return Object.freeze({schema:"career-mode-showdown.physical-journey-acceptance.v1",generatedAt:now(),appVersion:appVersion(),runtimeRevision:revision(),acceptanceMode:true,physicalJourneyMode:true,sanitizedSessionStorageOnly:true,recorderNetworkRequests:false,rawAuthorityIncluded:false,canonicalRawIncluded:false,device:deviceFacts(),deviceLabel:safe.deviceLabel||null,networkLabel:safe.networkLabel||null,managerRole:safe.managerRole,remoteRole:safe.remoteRole,accountFingerprint:safe.accountFingerprint,deviceFingerprint:safe.deviceFingerprint,rivalryFingerprint:safe.rivalryFingerprint,sessionFingerprints:[...safe.sessionFingerprints],authorityViolation:safe.authorityViolation,canonicalStorageProofScope:safe.canonicalStorageProofScope,canonicalStorageBeforeHash:safe.canonicalStorageBeforeHash,canonicalStorageAfterHash:safe.canonicalStorageAfterHash,canonicalStorageViolation:safe.canonicalStorageViolation,candidateCApplied:safe.candidateCApplied,offlineObserved:safe.offlineObserved,onlineRecovered:safe.onlineRecovered,reloadResumed:safe.reloadResumed,terminalReloadVerified:safe.terminalReloadVerified,conflictGuardProven:safe.conflictGuardProven,startupCount:safe.startupCount,milestones:safe.milestones.map(item=>({...item})),completed:completed()});}
  ...
  function rows(){const stages=new Set(safe.milestones.map(item=>item.stage)),previewReady=safe.milestones.some(item=>item.stage==="local-reconciliation-safe"&&item.phase==="PREVIEW_READY");return [
    ["ACTIVE",stages.has("remote-active")],["SETUP",stages.has("setup-confirmed")],["CAREER",stages.has("career-start-ready")],["TRANSFER",stages.has("transfer-completed")],["RESULTS",stages.has("results-ready")],["COMMIT",stages.has("season-acknowledged")],["SCORING",stages.has("scoring-reconciled")],["HISTORY",stages.has("history-converged")],["CONFLICT",stages.has("conflict-guard-proven")],["AUTHORITY",safe.authorityViolation!==true],["OFFLINE RECOVERY",safe.offlineObserved&&safe.onlineRecovered],["RELOAD",safe.reloadResumed],["LOCAL RECON",previewReady&&storageProofComplete()],["FINAL",stages.has("final-season-reconciled")],["CLOSED",stages.has("terminal-closed")],["CLOSED AFTER RELOAD",safe.terminalReloadVerified]
  ];}
  function ensurePanel(){
    ...
    panel.append(create("strong","","MDP PHYSICAL JOURNEY"),create("p","","Acceptance-only observer. It records sanitized fingerprints and milestones; its canonical storage proof is scoped only to the read-only Local Reconciliation preview."));
  ...
  return Object.freeze({contractVersion:2,feature:"mdp-physical-journey-acceptance-recorder",enabled,acceptanceOnly:true,productionObserver:true,runtimeRevision:"1.9.1-r43",pollIntervalMs:POLL_MS,sanitizedSessionStorageOnly:true,rawAuthorityPersistence:false,providerWriteRequired:false,recorderNetworkRequests:false,canonicalStorageMutation:false,canonicalStorageProofScope:"local-reconciliation-preview",candidateCAutomaticApply:false,billingRequired:false,blazeRequired:false,cloudRunRequired:false,cloudFunctionsRequired:false,install,destroy,observe,setLabels,captureLocalReconciliationBaseline,verifyLocalReconciliationPreview,runConflictGuardProbe:async()=>safeConflictProbe(await bindIdentity()),getEvidence:evidence,copyEvidence,downloadEvidence,reset,getState:()=>Object.freeze({enabled,installed,completed:completed(),startupCount:safe.startupCount,managerRole:safe.managerRole,milestones:safe.milestones.length,authorityViolation:safe.authorityViolation,offlineObserved:safe.offlineObserved,onlineRecovered:safe.onlineRecovered,reconnectRecoveredStartupCount:safe.reconnectRecoveredStartupCount,reloadResumed:safe.reloadResumed,terminalReloadVerified:safe.terminalReloadVerified,conflictGuardProven:safe.conflictGuardProven,canonicalStorageProofReady:storageProofComplete()})});
});