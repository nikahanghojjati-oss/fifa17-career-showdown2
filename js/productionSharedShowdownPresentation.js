(function(root,factory){
  ...
  async function ensureGameplay(){if(typeof root.ensureGameplayModules!=="function")throw new Error("Gameplay presentation loader is unavailable.");await root.ensureGameplayModules();if(typeof root.ensureRequiredFootballVisualExperience==="function")await root.ensureRequiredFootballVisualExperience();}
  ...
  function setSharedStatus(message){
    let node=root.document&&root.document.getElementById(STATUS_ID);const screen=root.document&&root.document.getElementById(activeScreen());if(!screen)return;
    if(!node){node=root.document.createElement("p");node.id=STATUS_ID;node.className="stateNote";node.setAttribute("role","status");node.setAttribute("aria-live","polite");const container=screen.querySelector(".wheelContainer")||screen.querySelector(".clubAssignmentStage")||screen;container.insertBefore(node,container.firstChild);}text(node,message);
  }
  ...
  function clubDigest(setup){return setup&&setup.clubs?`${setup.leagueId}|${setup.clubs.playerOne}|${setup.clubs.playerTwo}`:null;}
  ...
  function renderLeague(){
    ...
    if(state.setup.phase==="SHARED_SETUP_OPEN"){text(result,"Spin to reveal the shared league");setControl(button,{label:coordinator()?"SPIN SHARED LEAGUE WHEEL":"WAITING FOR HOST SPIN",disabled:!coordinator()});setSharedStatus(coordinator()?"AUTHORITATIVE SHARED SETUP OPEN · Spin the real league wheel.":"AUTHORITATIVE SHARED SETUP OPEN · Waiting for the host spin. This screen will update automatically.");return true;}
    ...
    if(witnessedLeagueId!==state.setup.leagueId){
      ...
      if(track){const rotation=typeof root.getLeagueRotation==="function"?root.getLeagueRotation(state.setup.leagueId,6):(typeof getLeagueRotation==="function"?getLeagueRotation(state.setup.leagueId,6):0);if(typeof root.setLeagueWheelTransition==="function")root.setLeagueWheelTransition(track,4000);else track.style.transition="transform 4000ms cubic-bezier(.16,.76,.16,1)";track.style.transform=`rotate(${rotation}deg)`;track.dataset.sharedLeagueId=state.setup.leagueId;later(()=>{const finalRotation=typeof root.getLeagueRotation==="function"?root.getLeagueRotation(state.setup.leagueId,0):(typeof getLeagueRotation==="function"?getLeagueRotation(state.setup.leagueId,0):rotation);if(typeof root.setLeagueWheelTransition==="function")root.setLeagueWheelTransition(track,0);else track.style.transition="none";track.style.transform=`rotate(${finalRotation}deg)`;},4100);}
    }
    setControl(button,{label:"CONTINUE TO CLUB PACKS",disabled:false});if(note){text(note,`${league&&league.name||state.setup.leagueId} is authoritative and locked for both managers.`);note.classList.remove("hidden");}setSharedStatus("LEAGUE REVEALED ON THIS DEVICE · Continue to the original club-pack screen. No reroll.");return true;
  }

  function resetPackCards(){if(typeof root.resetClubRevealCards==="function")root.resetClubRevealCards();else for(const [cardId,nameId,stateId] of [["clubCardOne","clubNameOne","clubCardStateOne"],["clubCardTwo","clubNameTwo","clubCardStateTwo"]]){const card=root.document.getElementById(cardId),name=root.document.getElementById(nameId),stateNode=root.document.getElementById(stateId);if(card)card.classList.remove("is-revealed");text(name,"?");text(stateNode,"SEALED");}}
  function revealCard(which,name){const card=root.document.getElementById(which===1?"clubCardOne":"clubCardTwo"),nameNode=root.document.getElementById(which===1?"clubNameOne":"clubNameTwo"),stateNode=root.document.getElementById(which===1?"clubCardStateOne":"clubCardStateTwo");if(typeof root.applyClubRevealCard==="function")root.applyClubRevealCard(card,nameNode,stateNode,name);else{text(nameNode,name);text(stateNode,"REVEALED");if(card)card.classList.add("is-revealed");if(typeof root.applyClubIdentity==="function")root.applyClubIdentity(nameNode,name);}}
  function ensureSeasonPanel(){
    ...
    for(const seasons of LENGTHS){const button=root.document.createElement("button");button.type="button";button.className="compactButton";button.dataset.sharedSeason=String(seasons);button.textContent=`${seasons} SEASON${seasons===1?"":"S"}`;button.addEventListener("click",event=>{event.preventDefault();event.stopPropagation();void chooseSeason(seasons);});choices.append(button);}panel.append(eyebrow,heading,copy,choices);const confirmation=root.document.getElementById("clubRivalryConfirmation");if(confirmation)confirmation.insertAdjacentElement("afterend",panel);else screen.append(panel);return panel;
  }
  function fillConfirmation(setup){const m=managers(),league=leagueRecord(setup.leagueId),confirmation=root.document.getElementById("clubRivalryConfirmation");if(confirmation)confirmation.classList.remove("hidden");text(root.document.getElementById("clubConfirmationShowdown"),(shell()&&shell().name)||"SHARED SHOWDOWN");text(root.document.getElementById("clubConfirmationMeta"),`${league&&league.name||setup.leagueId}${setup.totalSeasons?` · ${setup.totalSeasons} season${setup.totalSeasons===1?"":"s"}`:""} · SHARED`);text(root.document.getElementById("clubConfirmationManagerOne"),m.playerOne||"PLAYER ONE");text(root.document.getElementById("clubConfirmationManagerTwo"),m.playerTwo||"PLAYER TWO");text(root.document.getElementById("clubConfirmationClubOne"),setup.clubs&&setup.clubs.playerOne||"?");text(root.document.getElementById("clubConfirmationClubTwo"),setup.clubs&&setup.clubs.playerTwo||"?");if(typeof root.applyClubIdentity==="function"&&setup.clubs){root.applyClubIdentity(root.document.getElementById("clubConfirmationClubOne"),setup.clubs.playerOne);root.applyClubIdentity(root.document.getElementById("clubConfirmationClubTwo"),setup.clubs.playerTwo);}}
  function completePackWitness(setup){const digest=clubDigest(setup);witnessedClubDigest=digest;clubRevealComplete=true;const screen=root.document&&root.document.getElementById("clubWheelScreen");if(screen)screen.dataset.sharedClubPacksWitnessed=digest||"true";fillConfirmation(setup);void renderClub();}
  function animatePacks(setup){
    if(!setup||!setup.clubs)return;const screen=root.document.getElementById("clubWheelScreen");if(!screen)return;const digest=clubDigest(setup);
    if(witnessedClubDigest===digest&&clubRevealComplete){revealCard(1,setup.clubs.playerOne);revealCard(2,setup.clubs.playerTwo);fillConfirmation(setup);return;}
    clearTimers();clubRevealComplete=false;screen.dataset.sharedPackDigest=digest;resetPackCards();if(typeof root.setClubRevealStage==="function")root.setClubRevealStage("opening");text(root.document.getElementById("clubPackStatus"),"AUTHORITATIVE CLUB DRAW LOCKED · OPENING PACK 01");const reduced=typeof root.isReducedClubMotionPreferred==="function"&&root.isReducedClubMotionPreferred();
    if(reduced){revealCard(1,setup.clubs.playerOne);revealCard(2,setup.clubs.playerTwo);if(typeof root.setClubRevealStage==="function")root.setClubRevealStage("confirmation");completePackWitness(setup);return;}
    ...
    later(()=>{if(typeof root.setClubRevealStage==="function")root.setClubRevealStage("confirmation");text(root.document.getElementById("clubPackStatus"),"BOTH CLUBS REVEALED · SHARED RIVALRY LOCKED");completePackWitness(setup);},3000);
  }
  function renderSeasonControls(setup){const panel=ensureSeasonPanel();if(!panel)return;const copy=panel.querySelector("[data-shared-season-copy]"),choices=Array.from(panel.querySelectorAll("[data-shared-season]"));if(!clubRevealComplete){panel.classList.add("hidden");return;}if(setup.phase==="CLUB_ASSIGNMENTS_COMMITTED"){panel.classList.remove("hidden");text(copy,coordinator()?"Choose how many seasons this shared rivalry will run. This locks for both managers.":"Waiting for the coordinator to choose 1, 3, 5, or 10 seasons.");for(const button of choices){button.classList.remove("hidden");button.disabled=!coordinator()||busy;}return;}if(phaseAtLeast("SEASON_LENGTH_COMMITTED")){panel.classList.remove("hidden");text(panel.querySelector("h3"),`${setup.totalSeasons} SEASON${setup.totalSeasons===1?"":"S"} LOCKED`);text(copy,"Season length is authoritative and identical for both managers.");for(const button of choices)button.classList.add("hidden");return;}panel.classList.add("hidden");}
  function renderConfirmButton(setup){const button=root.document.getElementById("continueClubAssignment");if(!button)return;if(!clubRevealComplete){setControl(button,{label:"WATCH BOTH PACK REVEALS",disabled:true,hidden:true});return;}if(setup.phase==="SEASON_LENGTH_COMMITTED"){const confirmed=setup.confirmedRoles.includes(state.managerRole);setControl(button,{label:confirmed?"CONFIRMED · WAITING FOR RIVAL":"CONFIRM SHARED SHOWDOWN",disabled:confirmed||busy,hidden:false});return;}if(setup.phase==="SHOWDOWN_CONFIRMED"){setControl(button,{label:"SHARED SHOWDOWN READY ✓",disabled:true,hidden:false});return;}setControl(button,{label:"CONFIRM SHARED SHOWDOWN",disabled:true,hidden:true});}
  function renderClub(){
    if(!root.document||!pending())return false;removeForeignStatus();const setup=state&&state.setup;if(!setup||!phaseAtLeast("LEAGUE_WHEEL_COMMITTED"))return false;if(witnessedLeagueId!==setup.leagueId){forceScreen("leagueWheelScreen");return renderLeague();}
    const screen=root.document.getElementById("clubWheelScreen"),league=leagueRecord(setup.leagueId),m=managers();if(!screen)return false;screen.dataset.sharedPresentationRole=state&&state.managerRole||"unresolved";text(root.document.getElementById("clubAssignmentLeague"),league&&league.name||setup.leagueId);text(root.document.getElementById("clubPlayerOne"),m.playerOne||"PLAYER ONE");text(root.document.getElementById("clubPlayerTwo"),m.playerTwo||"PLAYER TWO");const open=root.document.getElementById("openClubPack"),back=root.document.getElementById("clubAssignmentBack");if(back){back.disabled=true;back.classList.add("hidden");}
    if(setup.phase==="LEAGUE_WHEEL_COMMITTED"){clubRevealComplete=false;witnessedClubDigest=null;resetPackCards();const confirmation=root.document.getElementById("clubRivalryConfirmation");if(confirmation)confirmation.classList.add("hidden");const panel=root.document.getElementById(SEASON_PANEL_ID);if(panel)panel.classList.add("hidden");text(root.document.getElementById("clubPackStatus"),"LEAGUE LOCKED · TWO SHARED CLUB PACKS READY");setControl(open,{label:coordinator()?"OPEN SHOWDOWN PACKS":"WAITING FOR HOST PACK REVEAL",disabled:!coordinator()||busy,hidden:false});renderConfirmButton(setup);setSharedStatus(coordinator()?"AUTHORITATIVE CLUB PACKS · Open the original two-pack reveal.":"AUTHORITATIVE CLUB PACKS · Waiting for the host. These packs will open automatically when the provider commits the clubs.");return true;}
    setControl(open,{label:"PACKS OPENED",disabled:true,hidden:true});animatePacks(setup);fillConfirmation(setup);renderSeasonControls(setup);renderConfirmButton(setup);if(setup.phase==="SHOWDOWN_CONFIRMED")setSharedStatus(clubRevealComplete?"SHARED SETUP COMPLETE · Both managers witnessed the league wheel and club packs on this device.":"SHARED SETUP COMPLETE IN AUTHORITY · Finish watching both pack reveals on this device.");else if(setup.phase==="SEASON_LENGTH_COMMITTED")setSharedStatus(clubRevealComplete?"FINAL CONFIRMATION · Each manager confirms on their own device.":"WATCH BOTH CLUB PACK REVEALS · Confirmation unlocks after this device witnesses them.");else setSharedStatus(clubRevealComplete?"CLUBS REVEALED · Choose the shared season length below.":"OPENING AUTHORITATIVE CLUB PACKS · Both managers see the same two reveals.");return true;
  }
  ...
  async function poll(){if(!active||!pending()||busy||pollBusy||root.document&&root.document.visibilityState==="hidden")return;pollBusy=true;try{await refresh();}catch(_error){}finally{pollBusy=false;}}
  function startPolling(){if(pollTimer!==null||typeof root.setInterval!=="function")return;pollTimer=root.setInterval(()=>void poll(),POLL_MS);}
  function stopPolling(){if(pollTimer!==null){root.clearInterval(pollTimer);pollTimer=null;}}
  async function mutate(type,extra){if(busy)return false;busy=true;try{const api=await ensureSetup(),result=await api.mutate(type,extra||{});state=api.getState();await renderCurrent();return result&&result.ok===true;}finally{busy=false;await renderCurrent();}}
  async function chooseSeason(seasons){if(!active||!pending()||!clubRevealComplete||!coordinator()||!state.setup||state.setup.phase!=="CLUB_ASSIGNMENTS_COMMITTED")return false;return mutate("commit-length",{totalSeasons:Number(seasons)});}
  ...
  async function handleControlClick(id){
    ...
    if(id==="spinLeague"){
      ...
      if(state.setup&&phaseAtLeast("LEAGUE_WHEEL_COMMITTED")){if(witnessedLeagueId!==state.setup.leagueId){await renderLeague();return true;}forceScreen("clubWheelScreen");await renderClub();return true;}return true;
    }
    ...
    if(id==="continueClubAssignment"){if(!clubRevealComplete)return true;if(state.setup&&state.setup.phase==="SEASON_LENGTH_COMMITTED"&&!state.setup.confirmedRoles.includes(state.managerRole)){await mutate("confirm");return true;}await refresh();return true;}
    return false;
  }
  async function activate(){active=true;await ensureGameplay();await ensureSetup();await refresh();const plain=root.document&&root.document.getElementById("productionSharedSetupOverlay");if(plain)plain.classList.add("hidden");forceScreen("leagueWheelScreen");await renderLeague();startPolling();return true;}
  function deactivate(){active=false;stopPolling();clearTimers();const panel=root.document&&root.document.getElementById(SEASON_PANEL_ID);if(panel)panel.remove();const note=root.document&&root.document.getElementById(STATUS_ID);if(note)note.remove();return true;}
  function install(){if(installed)return true;installed=true;return true;}

  return Object.freeze({contractVersion:2,feature:"ssjr-production-shared-showdown-polished-presentation",productionEnabled:true,pairingRequired:true,exactActiveSessionRequired:true,providerOwnsDrawAuthority:true,bothManagerRolesWitnessLeagueWheel:true,bothManagerRolesWitnessClubPacks:true,peerAutoRefreshesAuthority:true,usesLeagueWheelScreen:true,usesClubPackRevealScreen:true,engineeringSetupPanelPlayerFacing:false,localRandomLeagueAuthority:false,localRandomClubAuthority:false,canonicalStorageMutation:false,billingRequired:false,blazeRequired:false,cloudRunRequired:false,cloudFunctionsRequired:false,appCheckEnforcementRequired:false,install,activate,deactivate,refresh,handleControlClick,handlesControl,renderCurrent,isPresentationActive:()=>active&&pending(),getState:()=>Object.freeze({active:active&&pending(),phase:state&&state.setup&&state.setup.phase||null,revision:state&&state.setup&&state.setup.revision||0,managerRole:state&&state.managerRole||null,route:activeScreen(),leagueWitnessed:witnessedLeagueId,clubPacksWitnessed:witnessedClubDigest,clubRevealComplete})});
});
