(function(root,factory){
  ...
  function ssjpResetWitnesses(){
    ssjpClearTimers();witnessedLeagueId=null;witnessedClubDigest=null;revealingClubDigest=null;clubRevealComplete=false;preparedSeasonCommitPromise=null;
  ...
  function ssjpRenderLeague(){
    ...
    if(!screen||!button||!result)return false;screen.dataset.sharedPresentationRole=state&&state.managerRole||"unresolved";if(state?.setup?.phase)screen.dataset.sharedSetupPhase=state.setup.phase;else delete screen.dataset.sharedSetupPhase;
    ...
    ssjpSetControl(button,{label:"CONTINUE TO CLUB PACKS",disabled:ssjpActionBusy()});if(note){ssjpText(note,`${league&&league.name||state.setup.leagueId} is authoritative and locked for both managers.`);note.classList.remove("hidden");}ssjpSetSharedStatus("LEAGUE REVEALED ON THIS DEVICE · Continue to the original club-pack screen. No reroll.");return true;
  }
  ...
  function ssjpRenderSeasonControls(setup){
    ...
    if(!clubRevealComplete){panel.dataset.sharedSeasonState="hidden";panel.classList.add("hidden");return;}
    if(setup.phase==="CLUB_ASSIGNMENTS_COMMITTED"){
      panel.classList.remove("hidden");
      const local=ssjpLocalSeasonLength(),bound=ssjpBoundSeasonLength(),mismatch=Boolean(local&&bound&&local!==bound);panel.dataset.sharedSeasonState=mismatch?"mismatch":prepared?"locking":"recovery";
    ...
    if(ssjpPhaseAtLeast("SEASON_LENGTH_COMMITTED")){
      panel.classList.remove("hidden");
      const matches=ssjpSeasonMatchesPrepared(setup);panel.dataset.sharedSeasonState=matches?"locked":"mismatch";
    ...
    panel.dataset.sharedSeasonState="hidden";panel.classList.add("hidden");
  }
