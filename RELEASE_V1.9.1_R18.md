...
After the seal reaches the configured final season, the provider atomically closes the exact ACTIVE private session and parent rivalry. The final Rules path accepts only the terminal fields as changed, binds the closed session revision into `terminalProgress`, and validates the exact r18 witness against the Rules-backed accumulated totals. The persisted terminal witness intentionally contains only facts that production Rules can independently verify. r17 `acceptedRevisionKey` and fixed-club metadata remain part of Final Reconciliation but are not persisted as Terminal Close authority because Rules cannot independently reconstruct those browser-side provenance values. Canonical local Save storage is never mutated by Terminal Close.
