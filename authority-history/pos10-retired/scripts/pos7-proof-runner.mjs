...
const graph = JSON.parse(fs.readFileSync(path.join(root,'POS7_IMPACT_GRAPH.json'),'utf8'));
...
const commands = {
  ...
  STAGE4_RECONCILIATION_BROWSER: `CMS_AUDIT_RUN="pos7-impact-remote" node tests/browser/stage4-remote-local-reconciliation-audit.cjs`,
  ...
  BACKUP_BROWSER: `CMS_AUDIT_RUN="pos7-impact-storage" npm run test:backup-browser`,
  IMPORT_BROWSER: `CMS_AUDIT_RUN="pos7-impact-storage" npm run test:import-browser`,
  RESTORE_BROWSER: `CMS_AUDIT_RUN="pos7-impact-storage" npm run test:restore-browser`,
  MULTI_SAVE_BROWSER: `CMS_AUDIT_RUN="pos7-impact-storage" npm run test:multi-save-browser`,
  HOME_VISUAL: `CMS_AUDIT_RUN="pos7-impact-visual" npm run test:home-visual`,
  LOADING_VISUAL: `CMS_CHROMIUM_MULTI_CONTEXT=1 CMS_AUDIT_RUN="pos7-impact-visual" npm run test:loading-visual`,
  FOOTBALL_VISUAL: `CMS_AUDIT_RUN="pos7-impact-visual" npm run test:football-visual`,
  SHARED_POLISHED_PRESENTATION_BROWSER: `CMS_CHROMIUM_MULTI_CONTEXT=1 CMS_AUDIT_RUN="pos7-impact-visual" node tests/browser/shared-showdown-polished-presentation-audit.cjs`,
  RUNTIME_BOUNDARY_BROWSER: `CMS_AUDIT_RUN="pos7-impact-full" npm run test:runtime-boundary`,
  SAVE_LIBRARY_UI_BROWSER: `CMS_AUDIT_RUN="pos7-impact-full" node tests/browser/save-library-ui-audit.cjs`,
  MANAGER_IDENTITY_BROWSER: `CMS_AUDIT_RUN="pos7-impact-full" node tests/browser/manager-identity-linkage-audit.cjs`,
  IDENTITY_SAFE_ANALYTICS_BROWSER: `CMS_AUDIT_RUN="pos7-impact-full" node tests/browser/identity-safe-career-analytics-audit.cjs`,
  OFFLINE_BOUNDARY_BROWSER: `CMS_AUDIT_RUN="pos7-impact-full" node tests/browser/offline-boundary-audit.cjs`,
  OFFLINE_CACHE_LIFECYCLE_BROWSER: `CMS_CHROMIUM_MULTI_CONTEXT=1 CMS_AUDIT_RUN="pos7-impact-full" node tests/browser/offline-cache-lifecycle-v2-audit.cjs`,
  FULL_BROWSER_BUNDLE: `CMS_AUDIT_RUN="pos7-impact-full" npm run test:browser`
};
...
function run(command,label){
  process.stdout.write(`RUN IMPACT-7 proof ${label}\n`);
...
async function main(){
  ...
  if(unknown.length) throw new Error(`Unknown IMPACT-7 proof IDs: ${unknown.join(', ')}`);
  if(group) requested = requested.filter(id=>groupFor(id) === group);
  if(!requested.length){
    console.log(`PASS IMPACT-7 proof runner: no proofs selected${group ? ` for group ${group}` : ''}.`);
    return;
  }
  ...
  try{
    if(group && serverGroups.has(group)){
      logFd = fs.openSync(`/tmp/cms-pos7-${group.toLowerCase()}-server.log`,'w');
  ...
  console.log(`PASS IMPACT-7 proof group ${group || 'UNSCOPED'} (${requested.length} exact proof IDs).`);
}
