...
function cacheNameForRevision(revision){
    return revision ? `${CACHE_PREFIX}${revision}` : "";
}

function requestForShellPath(path){
    return new Request(versionedShellUrl(path), { cache: "reload", credentials: "same-origin" });
}
...
async function verifyCache(revision = RUNTIME_REVISION){
    ...
    if(!revision || !(await cacheExists(cacheName))){
        return { ok: false, available: false, cacheName, revision, expected: SHELL_PATHS.length, missing: SHELL_PATHS.slice() };
    }
    const cache = await caches.open(cacheName);
    const missing = [];
    for(const path of SHELL_PATHS){
        const response = await cache.match(versionedShellUrl(path, revision));
        if(!response || !response.ok){ missing.push(path); }
    }
...
async function populateCurrentCache(){
    ...
    try{
        await cache.addAll(SHELL_PATHS.map(requestForShellPath));
        const status = await verifyCache(RUNTIME_REVISION);
        if(!status.ok){ throw new Error(`Application shell cache is incomplete: ${status.missing.join(", ")}`); }
        return status;
    }catch(error){
...
async function readForcedRevision(){
    ...
    const cache = await caches.open(MODE_CACHE_NAME);
    const response = await cache.match(scopeUrl("__cms_runtime_mode__").href);
    if(!response){ return ""; }
    try{ return (await response.text()).trim(); }catch(error){ return ""; }
}

async function writeForcedRevision(revision){
    const cache = await caches.open(MODE_CACHE_NAME);
    await cache.put(scopeUrl("__cms_runtime_mode__").href, new Response(String(revision || ""), { headers: { "content-type": "text/plain; charset=utf-8" } }));
}
...
async function chooseNavigationRuntime(){
    ...
    if(PREVIOUS_RUNTIME_REVISION){
        const previousStatus = await verifyCache(PREVIOUS_RUNTIME_REVISION);
        if(previousStatus.ok){ return previousStatus; }
    }
    return null;
}

async function getStatusBundle(){
    const current = await verifyCache(RUNTIME_REVISION);
    const previous = PREVIOUS_RUNTIME_REVISION ? await verifyCache(PREVIOUS_RUNTIME_REVISION) : { ok: false, available: false, cacheName: "", revision: "", expected: SHELL_PATHS.length, missing: SHELL_PATHS.slice() };
...
self.addEventListener("message", event => {
    ...
    if(type === "CMS_GET_CACHE_STATUS"){
        event.waitUntil((async () => {
            try{
                const status = await getStatusBundle();
                replyToClient(event, { type: "CMS_CACHE_STATUS", ok: status.current.ok, revision: RUNTIME_REVISION, previousRevision: PREVIOUS_RUNTIME_REVISION, ...status });
            }catch(error){
                replyToClient(event, { type: "CMS_CACHE_STATUS", ok: false, revision: RUNTIME_REVISION, error: error?.message || String(error) });
            }
        })());
        return;
    }
    ...
    if(type === "CMS_ROLLBACK_TO_PREVIOUS"){
        event.waitUntil((async () => {
            try{
                if(!PREVIOUS_RUNTIME_REVISION){ throw new Error("No previous application shell is available for rollback."); }
                const previous = await verifyCache(PREVIOUS_RUNTIME_REVISION);
                if(!previous.ok){ throw new Error(`Previous application shell is incomplete: ${previous.missing.join(", ")}`); }
                await writeForcedRevision(PREVIOUS_RUNTIME_REVISION);
                replyToClient(event, { type: "CMS_ROLLBACK_ACCEPTED", ok: true, revision: PREVIOUS_RUNTIME_REVISION });
            }catch(error){
                replyToClient(event, { type: "CMS_ROLLBACK_REJECTED", ok: false, error: error?.message || String(error) });
            }
        })());
        return;
    }
    if(type === "CMS_CLEAR_ROLLBACK"){
        event.waitUntil((async () => {
            await clearForcedRevision();
            replyToClient(event, { type: "CMS_ROLLBACK_CLEARED", ok: true, revision: RUNTIME_REVISION });
        })());
    }
});
...
self.addEventListener("activate", event => {
    event.waitUntil((async () => {
        const status = await verifyCache(RUNTIME_REVISION);
        if(!status.ok){ throw new Error(`Refusing activation with incomplete application shell: ${status.missing.join(", ")}`); }
        await clearForcedRevision();
        const keepShellCaches = new Set([CACHE_NAME, PREVIOUS_CACHE_NAME].filter(Boolean));
        const cacheNames = await caches.keys();
        await Promise.all(cacheNames.map(name => {
            if(name.startsWith(CACHE_PREFIX) && !keepShellCaches.has(name)){ return caches.delete(name); }
            if(name.startsWith(MODE_CACHE_PREFIX) && name !== MODE_CACHE_NAME){ return caches.delete(name); }
...
async function cachedShellResponse(path, revision){
    const cacheName = cacheNameForRevision(revision);
    if(!cacheName || !(await cacheExists(cacheName))){ return null; }
    const cache = await caches.open(cacheName);
    return cache.match(versionedShellUrl(path, revision));
}
...
self.addEventListener("fetch", event => {
    ...
    if(request.mode === "navigate"){
        event.respondWith((async () => {
            const selected = await chooseNavigationRuntime();
            if(selected){
                const cached = await cachedShellResponse("index.html", selected.revision);
