...
Journey Reconnect now stays dormant during an ordinary healthy ACTIVE session when no recovery history exists. Normal Transfer Challenge play therefore does not poll Multi Season progression or surface `JOURNEY_RECONNECT_PROGRESSION_NOT_AUTHORITATIVE`. When a private session genuinely expires, the existing recovery flow still exposes `RECONNECT SESSION`; a fresh ACTIVE session can reauthorize the same durable rivalry without redrawing league/clubs or resetting accepted seasons.
