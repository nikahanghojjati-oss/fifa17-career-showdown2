...
(async()=>{
  const variants=[
    ["ALLOW_TRUE","true"],
    ["ENTITLED","ssjrEntitled(rivalryId)"],
    ...
    ["PREREQUISITES","ssjrTransferPrerequisites(rivalryId, request.resource.data.seasonNumber)"],
    ["SHAPE","ssjrTransferPublicShape(request.resource.data, rivalryId, transferId)"],
    ...
    ["NEW_OP_ID","ssjrTransferPublicShape(request.resource.data, rivalryId, transferId) && ssjrTransferPublicPrefixPreserved(resource.data, request.resource.data) && ssjrTransferValidOperationId(request.resource.data.operationIds[resource.data.revision])"],
    ["NEW_OP_UNIQUE","ssjrTransferPublicShape(request.resource.data, rivalryId, transferId) && ssjrTransferPublicPrefixPreserved(resource.data, request.resource.data) && !(request.resource.data.operationIds[resource.data.revision] in resource.data.operationIds)"],
    ["NEW_OP_TYPE","ssjrTransferPublicShape(request.resource.data, rivalryId, transferId) && ssjrTransferPublicPrefixPreserved(resource.data, request.resource.data) && (request.resource.data.operationTypes[resource.data.revision] == 'request-end-window' || request.resource.data.operationTypes[resource.data.revision] == 'advance-expired-window' || request.resource.data.operationTypes[resource.data.revision] == 'lock-guesses' || request.resource.data.operationTypes[resource.data.revision] == 'lock-signings')"],
    ["NEW_OP_HASH","ssjrTransferPublicShape(request.resource.data, rivalryId, transferId) && ssjrTransferPublicPrefixPreserved(resource.data, request.resource.data) && ssjrTransferValidHash(request.resource.data.operationHashes[resource.data.revision])"],
    ["NEW_OP_BASE","ssjrTransferPublicShape(request.resource.data, rivalryId, transferId) && ssjrTransferPublicPrefixPreserved(resource.data, request.resource.data) && request.resource.data.baseRevisions[resource.data.revision] == resource.data.revision"],
    ["NEW_OP_ACTOR","ssjrTransferPublicShape(request.resource.data, rivalryId, transferId) && ssjrTransferPublicPrefixPreserved(resource.data, request.resource.data) && request.resource.data.actorRoles[resource.data.revision] == ssjrActorRole(rivalryId)"],
    ["NEW_OPERATION_ONLY","ssjrTransferNewOperationValid(resource.data, request.resource.data)"],
    ["NEW_OP_CORE2","ssjrTransferPublicShape(request.resource.data, rivalryId, transferId) && ssjrTransferPublicPrefixPreserved(resource.data, request.resource.data) && ssjrTransferValidOperationId(request.resource.data.operationIds[resource.data.revision]) && !(request.resource.data.operationIds[resource.data.revision] in resource.data.operationIds)"],
    ["NEW_OP_CORE3","ssjrTransferPublicShape(request.resource.data, rivalryId, transferId) && ssjrTransferPublicPrefixPreserved(resource.data, request.resource.data) && ssjrTransferValidOperationId(request.resource.data.operationIds[resource.data.revision]) && !(request.resource.data.operationIds[resource.data.revision] in resource.data.operationIds) && request.resource.data.operationTypes[resource.data.revision] in ['request-end-window','advance-expired-window','lock-guesses','lock-signings']"],
    ["NEW_OP_CORE4","ssjrTransferPublicShape(request.resource.data, rivalryId, transferId) && ssjrTransferPublicPrefixPreserved(resource.data, request.resource.data) && ssjrTransferValidOperationId(request.resource.data.operationIds[resource.data.revision]) && !(request.resource.data.operationIds[resource.data.revision] in resource.data.operationIds) && request.resource.data.operationTypes[resource.data.revision] in ['request-end-window','advance-expired-window','lock-guesses','lock-signings'] && ssjrTransferValidHash(request.resource.data.operationHashes[resource.data.revision])"],
    ["NEW_OP_CORE5","ssjrTransferPublicShape(request.resource.data, rivalryId, transferId) && ssjrTransferPublicPrefixPreserved(resource.data, request.resource.data) && ssjrTransferValidOperationId(request.resource.data.operationIds[resource.data.revision]) && !(request.resource.data.operationIds[resource.data.revision] in resource.data.operationIds) && request.resource.data.operationTypes[resource.data.revision] in ['request-end-window','advance-expired-window','lock-guesses','lock-signings'] && ssjrTransferValidHash(request.resource.data.operationHashes[resource.data.revision]) && request.resource.data.baseRevisions[resource.data.revision] == resource.data.revision"],
    ["NEW_OP_CORE6","ssjrTransferPublicShape(request.resource.data, rivalryId, transferId) && ssjrTransferPublicPrefixPreserved(resource.data, request.resource.data) && ssjrTransferValidOperationId(request.resource.data.operationIds[resource.data.revision]) && !(request.resource.data.operationIds[resource.data.revision] in resource.data.operationIds) && request.resource.data.operationTypes[resource.data.revision] in ['request-end-window','advance-expired-window','lock-guesses','lock-signings'] && ssjrTransferValidHash(request.resource.data.operationHashes[resource.data.revision]) && request.resource.data.baseRevisions[resource.data.revision] == resource.data.revision && request.resource.data.actorRoles[resource.data.revision] == ssjrActorRole(rivalryId)"],
