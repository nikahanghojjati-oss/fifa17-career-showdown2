...
const continuity=()=>({repository:'o/r',continuityState:'READY',ownerRequestsTransfer:false,openAtomicUnits:1,unpublishedPackets:0});
...
test('caller cannot supply expected heads checkpoint or candidate validation as authority',()=>{for(const [key,value] of [['expectedCandidateHead',next],['checkpointHead',next],['candidateValidation','NOT_PUBLISHED'],['headsMatchExpectation',true]])assert.throws(()=>resolveLiveContinuity({...continuity(),[key]:value},authority(),fakeClient()),/Caller-controlled live authority field forbidden/);});
