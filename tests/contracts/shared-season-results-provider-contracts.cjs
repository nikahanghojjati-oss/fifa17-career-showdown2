...
function createHarness({transferRevision=6}={}){
  ...
  put(key('rivalries',rivalryId,'sharedSetup','authoritative'),{schemaVersion:1,objectType:'sharedSetupLedger',rivalryId,revision:6,phase:'SHOWDOWN_CONFIRMED',coordinatorRole:'playerOne',operationIds:[1,2,3,4,5,6].map(setupOp),operationTypes:['open','commit-league','commit-clubs','commit-length','confirm','confirm'],baseRevisions:[0,1,2,3,4,5],actorRoles:['playerOne','playerOne','playerOne','playerOne','playerOne','playerTwo'],totalSeasons:3,confirmedRoles:['playerOne','playerTwo'],activeSessionId:sessionId,updatedAt:ts(900_000),updatedByDeviceId:device2});
...
(async()=>{
  ...
  assert.deepEqual(h.store.get(p1Path).result,p1Result);assert.equal(h.store.has(p2Path),false);
  ...
  tampered.store.get(tamperedPath).result.leagueGoals=299;
  const tamperedRead=await Provider.read(tampered.options('playerOne',2_000_100));
  assert.equal(tamperedRead.ok,false);assert.equal(tamperedRead.code,'SEASON_RESULTS_PRIVATE_STATE_INVALID');

  console.log('PASS Shared Season Results Spark provider: exact account/device/rivalry/ACTIVE-session/setup/Career Start/completed Transfer Challenge prerequisites, repository-derived league team count, split public/private result storage, pre-ready opponent-read denial, role-owned immutable publication, CAS/idempotency, timeout and early-end transfer completion compatibility, private payload integrity, Spark-only zero billing and zero canonical Save mutation.');
