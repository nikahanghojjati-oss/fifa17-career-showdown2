...
assert.match(rules,/seasonIds\.size\(\) <= 10/i,"Shared-state authority must retain the product's ten-season resource bound.");
...
assert.match(emulator,/must not mutate canonical local saves/i,"Abuse proof must protect local-first recovery authority.");
...
process.stdout.write("PASS Stage 4 structural abuse-hardening contract: provider Rules retain non-enumerability, exact two-manager/ten-season resource bounds, pairing TTL limits, and permanent emulator proof wiring\n");
