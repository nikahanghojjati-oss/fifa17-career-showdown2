...
process.stdout.write("PASS Shared Canonical Scoring production contracts: r11 installs in the Shared Journey bootstrap, loads its r10 Season Commit provider prerequisite before the scoring provider factory, waits for provider-verified r10 ACKNOWLEDGED revision 3, refreshes exact account/device/rivalry/ACTIVE-session authority, renders both managers' canonical score breakdown and winner in the existing Season Review, and remains read-only with zero canonical local-save mutation and Spark-only zero billing.\n");
