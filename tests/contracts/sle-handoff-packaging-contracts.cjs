...
assert.equal(wec.lifecycle,"transition-prepared"); assert.deepEqual(wec,json(bootstrap.currentWec.archive)); assert.equal(wec.signals.handoffCompleteness,100); assert.equal(wec.environmentId,bootstrap.currentWec.environmentId); assert.equal(wec.repository.predecessorEnvironmentId,"we-2026-09-05-ssjr-setup-foundation-28cf84"); assert.equal(json(wec.repository.predecessorArchive).environmentId,wec.repository.predecessorEnvironmentId); assert.equal(bootstrap.transition.continuationDecision,wec.assessment.decision);
