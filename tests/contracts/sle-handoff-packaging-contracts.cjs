...
assert.equal(bootstrap.starter?.checkpoint,"PR207-POSTMERGE-GREEN-SSJR-PRODUCTION-TWO-ACCOUNT-NEXT");
assert.equal(bootstrap.starter?.canonical,"START_NEXT_SESSION_V1.4.52_PR207_POSTMERGE_GREEN_SSJR_PRODUCTION_TWO_ACCOUNT_NEXT.md");
assert.equal(bootstrap.currentHandoff?.canonical,"SUCCESSOR_HANDOFF_PR207_POSTMERGE_GREEN_SSJR_PRODUCTION_TWO_ACCOUNT_NEXT_SLE_2026-09-06.md");
...
const archived=json(bootstrap.currentWec.archive);assert.equal(archived.lifecycle,"transition-prepared");assert.equal(archived.signals.handoffCompleteness,100);assert.equal(archived.environmentId,"we-2026-09-06-ssjr-production-shared-setup-a51");assert.equal(archived.assessment.decision,"HANDOFF_NOW");assert.equal(archived.assessment.decisionInheritedFromPredecessor,false);assert.match(archived.assessment.reason,/PR207/i);
...
process.stdout.write("PASS SLE packaging: mirrored v1.4.52 PR207 recorder package, sealed a51 WEC or strict active successor, PR205 validator, PR203 r3 production authority, frozen RJR100/SSJR0 and genuine production-two-account successor task are protected.\n");
