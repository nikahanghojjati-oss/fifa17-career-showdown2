...
// runtime work superseded it; live authority is now PR213/r5 production-proven with fixed RJR100 accepted.
// PR203/r3 remains historical Shared Setup/bootstrap authority, PR194/r2 remains rollback provenance and PR198 remains the historical RJR100 publication checkpoint.
...
assert.match(state,/RJR-1[^\n]{0,40}100\/100|RJR100/i,"Current PROJECT_STATE must identify evidence-accepted RJR100 before immutable Stage 2A history.");
assert.match(state,/Production authority:[\s\S]{0,80}1\.9\.1-r5[\s\S]+PR #213/i,"Current PROJECT_STATE must identify PR213/r5 production authority.");
assert.match(state,/HISTORICAL COMPATIBILITY SNAPSHOT[\s\S]+1\.9\.1-r3[\s\S]+PR #203/i,"Current PROJECT_STATE must retain PR203/r3 as historical production Shared Setup/bootstrap provenance.");
assert.equal(currentProduction,true,"Current runtime provenance must identify production-proven PR #213 / v1.9.1-r5.");
assert.equal(historicalProductionR3,true,"Historical runtime provenance must preserve PR #203 / v1.9.1-r3.");
...
assert.equal(bootstrap.liveRuntime?.googleAuthPersistence,"browserSessionPersistence-popup-only-no-extra-scopes","Google Auth must remain popup-only browserSessionPersistence with no extra scopes.");
...
process.stdout.write("PASS Private Account/Auth Stage 2A emulator-only identity boundary with immutable historical selection/completion preserved while live PR213/r5 production, historical PR203/r3 bootstrap, r2 rollback, evidence-accepted RJR100 and PR198 publication authority remain explicit\n");