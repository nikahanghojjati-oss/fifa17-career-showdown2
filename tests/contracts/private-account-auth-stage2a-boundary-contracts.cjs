...
assert.match(next,/CURRENT OVERRIDE[\s\S]+PR #166 PRODUCTION ROLLBACK PROVEN[\s\S]+RJR85[\s\S]+PR #167 SLE TRANSITION/i,"Current NEXT_TASK must identify the PR #166 rollback / RJR85 / PR #167 transition authority rather than revive Stage 2A.");
assert.match(state,/CURRENT OVERRIDE[\s\S]+PR #166 PRODUCTION ROLLBACK PROVEN[\s\S]+RJR85[\s\S]+PR #167 SLE TRANSITION/i,"Current PROJECT_STATE must identify the PR #166 rollback / RJR85 / PR #167 transition authority rather than revive Stage 2A.");
...
assert.match(state,/Status:[\s\S]+production remains `v1\.8\.1 \/ 1\.8\.1-r5`/i,"Current PROJECT_STATE must expose restored r5 production truth.");
...
assert.match(state,/Production rollback proof workflow:\s*`33190961085` — SUCCESS/i,"Current PROJECT_STATE must retain exact production rollback proof provenance.");
...
assert.equal(readiness.currentScore,85,"Stage 2A current-state checks must preserve the fixed RJR85 rollback checkpoint.");
...
process.stdout.write("PASS Private Account/Auth Stage 2A emulator-only identity boundary with immutable historical selection/completion preserved and current PR #166 rollback / RJR85 / PR #167 transition authority explicit\n");
