...
assert.match(pairSource,/pairCurrentIdentityManagerId\(\)\|\|pairManagerForRole\(current\.binding\.managerRole\)\.id/,'Legacy rivalry migration must preserve the remembered named player independently of the old provider slot.');
...
assert.match(identitySource,/duplicate\.hidden=true/,'Legacy duplicate Shared Showdown action must be absent from the player surface.');
...
assert.match(rulesFragment,/cmsPersistentPairManagerValid\(pairData\.managerId\)/);
assert.match(rulesFragment,/managerId == 'daniel' \|\| managerId == 'nik'/,'Rules must treat named identity as immutable account metadata, not a provider-slot alias, so legacy slots remain recoverable.');
...
console.log('PASS single Showdown product language, Daniel/Nik identity mapping, legacy slot compatibility, lazy identity seeding and bounded production authority.');