...
const homeVisualCss=read('css/visual-fidelity-r3.css');
...
assert.match(pairSource,/stateNote r9PairPanel/,'R9 must style the real persistent Daniel\/Nik pair panel rather than create a duplicate pairing surface.');
assert.match(pairSource,/panel\.dataset\.pairState=state\.status\|\|"idle"/,'R9 pair presentation must project the existing provider-backed pair status.');
assert.match(pairSource,/r9PairActions/,'R9 pair presentation must preserve the existing real action container.');
assert.match(identitySource,/className="onlineIdentityGate"/,'R9 must style the existing online identity gate rather than add a second identity flow.');
assert.match(identitySource,/className="onlineIdentityCard"/,'R9 online identity presentation hook is missing.');
...
assert.match(rulesFragment,/let sameManager = after\.data\.managerRole == before\.data\.managerRole/,'Pair-link replacement must explicitly distinguish same-manager reuse from a post-terminal manager switch.');
assert.match(rulesFragment,/after\.data\.rivalryId == before\.data\.rivalryId[\s\S]*&& sameManager/,'The same live rivalry must never change its Daniel\/Nik assignment.');
assert.match(rulesFragment,/priorRivalry\.data\.data\.connectionState == 'closed'/,'Terminal Showdown authority must explicitly permit replacement of the account current-pair pointer, including a newly selected canonical manager.');
assert.match(rulesFragment,/sameManager[\s\S]*connectionState == 'pending-pair'/,'Expired pending-pair replacement must preserve the prior manager identity.');
const pairUpdateRules=rulesFragment.slice(rulesFragment.indexOf('function cmsPersistentPairUpdateValid'),rulesFragment.indexOf('// CMS_PERSISTENT_PAIR_FUNCTIONS_END'));
assert.doesNotMatch(pairUpdateRules,/&& after\.data\.managerRole == before\.data\.managerRole\s*&& after\.data\.managerId == before\.data\.managerId/,'Closed Showdowns must not permanently pin a Google account to its historical Daniel\/Nik role.');
