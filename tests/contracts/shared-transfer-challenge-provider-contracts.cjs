...
(async()=>{
  ...
  assert.match(providerSource,/type==="advance-expired-window"\)endedAt=serverNow/,"Timeout advancement must stamp endedAt from Firestore server time after the 15-minute gate.");
  ...
  assert.equal(timeoutResult.state.phase,'GUESS_ENTRY');assert.equal(timeoutResult.state.endedAtEpochMs,2_900_123,'provider projection must record the expiry transition attempt time rather than reconstructing the deadline');
