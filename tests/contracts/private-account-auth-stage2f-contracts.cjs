...
(async () => {
  ...
  assert.match(next,/STAGE 5 REMAINS LOCKED|Stage 5 host\/join\/session orchestration remains locked/i,"Live authority must keep actual Remote Joining sessions gated.");

  // Long-lived roadmaps preserve historical Stage 2E/2F provenance. Rolling handoff
  // documents instead prove the current r5 production transition.
  ...
  process.stdout.write("PASS Private Account/Auth Stage 2F trusted request authentication with immutable historical Stage 2E/2F transition preserved and current PR #171 provider-proven Rules / RJR86 / provider-abuse acceptance authority explicit\n");
})().catch(error => {
