...
assert.match(next, /App Check enforcement remains OFF[\s\S]+Firebase remains Spark \/ zero billing[\s\S]+Firestore remains memory-only/i,"Current NEXT_TASK must preserve controlled provider/runtime locks after the historical Phase 1F boundary.");
