...
assert.ok(source.settings.includes("DELETE CURRENT SHOWDOWN")&&source.settings.includes("ensureSettingsShowdownStorageAuthority")&&source.settings.includes("CareerModeSaveLibraryRuntime")&&source.settings.includes("clearActiveShowdown"),"Settings must expose a safe current-Showdown deletion route backed by established Save Library authority.");
assert.ok(source.settings.includes("closes the current Showdown connection for both players")&&source.settings.includes("keeps your player identity, registered device, Legacy history and app settings"),"Current-Showdown deletion must explain that the rivalry closes while identity, device, history and preferences are preserved.");
assert.ok(source.settings.includes("releaseSettingsCurrentShowdownConnection(authoritative)")&&source.settings.indexOf("releaseSettingsCurrentShowdownConnection(authoritative)")<source.settings.indexOf("runtime.clearActiveShowdown()"),"Provider Showdown authority must close before local active-save deletion.");
assert.ok(source.settings.includes("confirmedSaveId")&&source.settings.includes("authoritativeSaveId")&&source.settings.includes("The active Showdown changed after you confirmed deletion. Nothing was deleted."),"Settings deletion must re-confirm the exact Save identity after storage authority is reacquired so a concurrent tab cannot redirect a destructive confirmation.");
assert.ok(source.settings.includes('window.stopTransferTimerLoop')&&source.settings.includes('window.resetNavigationState')&&source.settings.includes('showScreen("mainMenu", false)')&&source.settings.includes('closeSettings(false)'),"Successful deletion must stop stale gameplay activity, clear navigation history, close Settings and return to Home.");
