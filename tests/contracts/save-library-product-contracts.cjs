...
assert.ok(source.onlineIdentity.includes('getElementById("saveLibraryProductPanel")')&&source.onlineIdentity.includes('p.hidden=true')&&source.onlineIdentity.includes('p.dataset.productSurface="internal"'),"Save Library must remain available as internal recovery architecture without being a normal player-facing mode.");
