...
assert.equal(productionEnvironment.activation.productionSecurityRulesSourceBlobSha,"2b7c0b166ae0aae7ab7a3ce84725b21091262484","The production manifest must retain the exact reviewed provider-published strengthened Rules blob.");
assert.match(productionEnvironment.activation.productionSecurityRulesVerificationEvidence,/2026-08-29[\s\S]+Today 7:48 AM[\s\S]+firestore\.spark\.rules[\s\S]+final allow read, write: if false/i,"The strengthened provider-verified Rules claim must retain its direct Firebase Console provenance.");
assert.match(providerProof,/Status: PROVIDER-VERIFIED DEPLOYED/i,"The dedicated provider proof must preserve deployed provider status.");
assert.match(providerProof,/Production project: `fifa17-career-showdown-prod`/i,"The dedicated provider proof must preserve the exact production project.");
assert.match(providerProof,/Database: `\(default\)`/i,"The dedicated provider proof must preserve the exact production database.");
assert.match(providerProof,/Today · 7:48 AM/i,"The dedicated provider proof must preserve the exact provider version timestamp.");
assert.match(providerProof,/Rules source: `firestore\.spark\.rules`/i,"The dedicated provider proof must preserve the strengthened Rules source.");
assert.match(providerProof,/2b7c0b166ae0aae7ab7a3ce84725b21091262484/i,"The dedicated provider proof must preserve the exact reviewed source blob regardless of document field order.");
