...
function observation(managerRole, remoteRole, suffix){
  ...
  return {
    schemaVersion:1,capturedAt:'2026-09-07T09:30:00Z',runtimeRevision:'1.9.1-r6',managerRole,remoteRole,
...
assert.equal(a.runtimeRevision,'1.9.1-r6');
assert.equal(b.runtimeRevision,'1.9.1-r6');
...
if (process.env.SKIP_PAIR_VALIDATOR !== '1') {
  ...
  const summary=JSON.parse(validated.stdout);assert.equal(summary.ok,true);assert.equal(summary.runtimeRevision,'1.9.1-r6');assert.equal(summary.finalRevision,6);assert.equal(summary.negativesProvenPerManager,8);
}
...
const staleRuntime=structuredClone(rawOne);staleRuntime.runtimeRevision='1.9.1-r5';
bad=record(staleRuntime);assert.notEqual(bad.status,0);assert.match(bad.stderr,/runtimeRevision must be 1\.9\.1-r6/i);
...
console.log('PASS SSJR production Shared Setup evidence recorder contracts: stdin-only raw authority is sanitized, exact r6 is enforced, closed-schema output is deterministic, and validator-compatible pair evidence is produced.');
