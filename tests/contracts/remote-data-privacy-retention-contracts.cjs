...
assert.match(next,/STAGE 4 RECONCILIATION PRODUCTION-PROVEN[\s\S]+v1\.8\.1 \/ 1\.8\.1-r4[\s\S]+App Check enforcement remains OFF/i,"Current NEXT_TASK must retain the later explicit controlled Firebase/App Check r4 runtime authority while advancing beyond the superseded r3 recovery incident.");
...
process.stdout.write("PASS Phase 1C remote data inventory, privacy, retention, anti-resurrection, deletion and local-only boundaries; historical non-runtime provenance is archived while current production-proven r4 reconciliation/token-lifecycle authority stays explicit\n");