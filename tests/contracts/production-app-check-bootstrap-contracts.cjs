...
assert.match(worker,/const APP_CHECK_BOOTSTRAP_PATH = "js\/productionAppCheckBootstrap\.js";/,"Service worker must identify the optional production App Check bootstrap separately from the offline shell.");
assert.match(worker,/if\(path===APP_CHECK_BOOTSTRAP_PATH\)\{return;\}/,"The lazy versioned App Check bootstrap must bypass shell-only service-worker interception so production can load it normally.");
