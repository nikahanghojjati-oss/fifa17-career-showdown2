...
assert.match(deploy,/grep -Fq "intent\.billingRequired == false" firestore\.terminal-close-production\.fragment\.rules/,"deployment guard must positively require Terminal Close to remain zero-billing");
assert.match(deploy,/! grep -Eqi "cloud\[\[:space:\]\]_-\]\*run\|cloud\[\[:space:\]\]_-\]\*functions\|blaze\|payment\|purchased\[\[:space:\]\]_-\]\*credits" firestore\.terminal-close-production\.fragment\.rules/,"Terminal Close deployment scan must ban paid compute without rejecting the explicit billingRequired=false safety invariant");
assert.doesNotMatch(deploy,/billing enable|firebase use --add|functions:deploy|run deploy/i);

console.log("PASS r18 Terminal Close production Rules: acknowledged seasons are folded into a bounded monotonic rivalry terminalProgress seal one season at a time, canonical scores are accumulated under Rules authority, persisted terminal witness fields are limited to provider-verifiable facts, terminal-owned writes are routed away from legacy pairing/session validators, the zero-billing deployment guard positively requires billingRequired=false while banning paid compute, and final ACTIVE-to-CLOSED still requires the exact session to close atomically with no list/delete/billing expansion.");