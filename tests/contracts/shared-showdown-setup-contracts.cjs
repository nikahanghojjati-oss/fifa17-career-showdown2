...
(async () => {
  ...
  const sparkProviderSource = fs.readFileSync("js/sparkSharedShowdownSetup.js","utf8");
  assert.match(sparkProviderSource,/function ssrpProtocolModule\(\)/,"Spark Shared Setup must resolve the protocol at call time in browser runtimes.");
  assert.match(sparkProviderSource,/function ssrpCatalogModule\(\)/,"Spark Shared Setup must resolve the catalog at call time in browser runtimes.");
  assert.doesNotMatch(sparkProviderSource,/const protocolModule=typeof require===/,"Spark Shared Setup must not freeze the browser protocol dependency at module evaluation.");
  assert.doesNotMatch(sparkProviderSource,/const catalogModule=typeof require===/,"Spark Shared Setup must not freeze the browser catalog dependency at module evaluation.");
