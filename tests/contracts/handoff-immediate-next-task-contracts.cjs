...
assert.notEqual(wec.environmentId,currentClosingWec.environmentId,"A successor must not inherit the closed predecessor identity.");
assert.equal(wec.environmentId,bootstrap.currentWec?.environmentId);
assert.equal(bootstrap.currentWec?.lifecycle,"active");
assert.equal(wec.repository?.predecessorEnvironmentId,currentClosingWec.environmentId);
...
process.stdout.write("PASS current immediate-next-task authority: immutable provenance remains sealed, production is v1.9.1-r2 at fixed RJR91, the predecessor WEC remains archived and the fresh publication WEC is active, and the fresh successor is routed through PR196 publication before genuine physical acceptance under zero-billing locks.\n");
