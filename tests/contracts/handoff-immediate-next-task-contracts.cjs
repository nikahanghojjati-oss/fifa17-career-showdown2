...
assert.equal(bootstrap.currentPublicationCheckpoint?.pullRequest,205);
assert.match(bootstrap.currentPublicationCheckpoint?.state,/postmerge-green.*evidence-validator-proven/i);
assert.equal(bootstrap.currentPublicationCheckpoint?.finalSealedHead,"55d1bcb5f88bb8dcd598090acbcee59887932a97");
assert.equal(bootstrap.currentPublicationCheckpoint?.exactTree,"72c6063793c8e2908f9b7175f57ad15f7b420d27");
assert.equal(bootstrap.currentPublicationCheckpoint?.mergeSha,"66abde6d51ade2e8fbe8296ba60ac46e18a2a353");
...
assert.equal(bootstrap.currentPublicationCheckpoint?.postMergeStabilityRunId,34033617877);
assert.equal(bootstrap.currentPublicationCheckpoint?.reviewP1ThreadsResolved,5);
...
assert.equal(bootstrap.currentWec?.environmentId,"we-2026-09-06-ssjr-production-shared-setup-a50");
assert.equal(bootstrap.currentWec?.startingMainSha,"bea3951e0295a9df5c532d1e22c3f8fd645d42a3");
assert.equal(bootstrap.currentWec?.finalMainSha,"66abde6d51ade2e8fbe8296ba60ac46e18a2a353");
...
process.stdout.write("PASS current authority: PR205 evidence validator tooling is reviewed/merged/post-merge green, PR203 remains r3 production runtime authority, RJR100 remains frozen, SSJR-1.1 remains 0/100, sealed a50 is bootstrap authority, and genuine production two-account evidence remains next without billing.\n");
