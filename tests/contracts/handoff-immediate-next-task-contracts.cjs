...
assert.ok(Number.isInteger(readiness.currentScore)&&readiness.currentScore>=86&&readiness.currentScore<=100,"The fixed RJR authority must preserve the provider-proven RJR86 checkpoint or later evidence.");
...
assert.match(next,/CURRENT OVERRIDE[\s\S]+PR #171[\s\S]+PROVIDER RULES PROVEN[\s\S]+RJR86[\s\S]+PROVIDER ABUSE ACCEPTANCE/i,"NEXT_TASK must expose current PR #171 provider-Rules / RJR86 / provider-abuse acceptance authority.");
assert.match(next,new RegExp(`Status:[\\s\\S]+v${escapeRegex(applicationVersion)} \\/ ${escapeRegex(productionRuntime)}[\\s\\S]+STAGE 5 REMAINS LOCKED`,"i"),"NEXT_TASK must identify the current production runtime and bounded Stage 5 lock.");
...
assert.match(next,/provider version history[\s\S]+Today · 7:48 AM[\s\S]+firestore\.spark\.rules[\s\S]+deny-by-default/i,"NEXT_TASK must record the direct provider-authoritative strengthened Rules proof.");
assert.match(next,/RJR ledger therefore advances exactly `85 → 86`[\s\S]+production-cloud-security[\s\S]+19\/20 → 20\/20/i,"NEXT_TASK must preserve the single bounded provider-publication credit.");
assert.match(next,/PR #169[\s\S]+PR #171 implementation[\s\S]+CI[\s\S]+documentation[\s\S]+zero duplicate credit/i,"NEXT_TASK must forbid process credit for the RJR86 transition.");
...
assert.match(next,/Production Authorization Acceptance[\s\S]+query-only[\s\S]+rivalries[\s\S]+limit\(1\)[\s\S]+permission denied/i,"NEXT_TASK must expose the bounded zero-write provider-abuse acceptance surface.");
assert.match(next,/Implementation, tests, merge and deployment alone do not prove production provider abuse acceptance/i,"NEXT_TASK must keep implementation/publication separate from live provider-abuse evidence.");
assert.match(next,/Authenticated third-account and revoked registered-device production negatives remain separately uncredited/i,"NEXT_TASK must keep unavailable state-dependent production negatives uncredited.");
...
assert.match(next,/Spark \/ zero billing[\s\S]+memory-only[\s\S]+popup-only `browserSessionPersistence`[\s\S]+App Check enforcement OFF[\s\S]+trusted-runtime IAM unactivated\/unbroadened/i,"NEXT_TASK must preserve provider, persistence, Auth, App Check and IAM locks.");
...
assert.match(next,/IMMEDIATE NEXT TASK AFTER FULL STUDY[\s\S]+finish PR #171 only[\s\S]+all 14 permanent workflow families[\s\S]+merge\/deploy under standing authorization[\s\S]+production `1\.8\.1-r5`/i,"NEXT_TASK must route the closing environment through exact-head PR #171 publication only.");
assert.match(next,/fresh WEC assessment after publication[\s\S]+PROBE ENUMERATION DENIAL[\s\S]+zero writes[\s\S]+Stage 5 lock/i,"NEXT_TASK must give the successor the concrete production provider-abuse acceptance and immediate Stage 5 reassessment.");
assert.match(next,/Do not repeat consumed[\s\S]+production-rollback proof/i,"NEXT_TASK must mark consumed proof as non-repeatable merely for confidence.");
...
assert.equal(bootstrap.currentPublicationCheckpoint?.pullRequest,171,"SESSION_BOOTSTRAP must identify PR #171 as the current publication checkpoint.");
...
assert.equal(bootstrap.currentPublicationCheckpoint?.providerAbuseProductionAcceptanceProven,false,"SESSION_BOOTSTRAP must not pre-credit the unexecuted live provider-abuse result.");
...
assert.equal(bootstrap.starter?.version,"1.4.29","The provider-Rules/RJR86 transition must publish starter v1.4.29.");
...
assert.equal(bootstrap.immediateNextTask?.name,"production-provider-abuse-enumeration-denial-acceptance","The successor bootstrap must route directly to the deployed zero-write provider-abuse acceptance after a fresh WEC.");
assert.match(bootstrap.immediateNextTask?.summary||"",/Fresh successor[\s\S]+PR #171[\s\S]+fresh WEC[\s\S]+PROBE ENUMERATION DENIAL[\s\S]+Stage 5/i,"The successor capsule must preserve publication verification, fresh WEC, production probe and immediate Stage 5 reassessment.");
...
assert.match(bootstrap.currentLane,/PR #171[\s\S]+firestore\.spark\.rules[\s\S]+RJR-1 to 86\/100[\s\S]+zero-write[\s\S]+stops before the separate owner-authenticated live provider-abuse acceptance/i,"Bootstrap current lane must preserve the provider-proven RJR86 boundary and transition-only stop rule.");
...
console.log(`Handoff immediate-next-task contracts passed: PR #171 provider-Rules/RJR86 transition tracks production ${applicationVersion}/${productionRuntime}, fixed RJR ${readiness.currentScore}/100, preserves permanent locks and nonclaims, and routes the fresh successor to the zero-write production provider-abuse acceptance after publication.`);