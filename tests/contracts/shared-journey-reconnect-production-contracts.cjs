...
assert.match(appCss,/#sharedJourneyReconnectStatus\[data-shared-journey-reconnect="true"\]/,'R9 must style the existing global Shared Journey reconnect status.');
assert.match(appCss,/#sharedJourneyReconnectStatus\[data-recovery-phase="OFFLINE_HOLD"\]/,'Offline hold must remain visually distinguishable from authoritative recovered state.');
assert.match(appCss,/#sharedJourneyReconnectStatus\[data-recovery-phase="ACTIVE_RECOVERED"\]\[data-authoritative="true"\]/,'Recovered success styling must require the existing authoritative projection.');
assert.match(appCss,/#sharedJourneyReconnectAction/,'R9 must style the existing reconnect action rather than invent a second recovery control.');
