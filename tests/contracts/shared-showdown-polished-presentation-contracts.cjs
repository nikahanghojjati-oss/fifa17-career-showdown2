...
assert.match(presentation,/setInterval\(\(\)=>void poll\(\),POLL_MS\)/);
assert.match(presentation,/if\(witnessedLeagueId!==setup\.leagueId\)\{forceScreen\("leagueWheelScreen"\);return renderLeague\(\);\}/);
...
assert.match(bridge,/openPanel:polishedOpen/);
