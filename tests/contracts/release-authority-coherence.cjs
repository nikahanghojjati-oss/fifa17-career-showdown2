...
for(const stale of [
    ...
    for(const [file, text] of [["NEXT_TASK.md", next], ["PROJECT_STATE.md", state], ["README.md", readme]]){
...
A.match(cloud, /future architecture contract only/i, "Cloud foundation must preserve its original non-runtime architecture boundary as historical authority.");
...
A.match(start, /explicit cross-Save\/historical manager identity linkage foundation — PR #57/i, "Developer bootstrap must include the shipped manager-identity layer.");
A.match(start, /identity-safe longitudinal Career Analytics \/ Trophy Room correction — PR #59/i, "Developer bootstrap must include the shipped Analytics layer.");
...
A.match(state, /Phase B first-slice production merge: `65b6c9db0a070b6e5e992a39dffeee23df0c6f08`/i, "PROJECT_STATE must identify the exact Phase B first-slice merge.");
A.match(state, /Phase C first-slice production merge: `dec1d3ba8182c3f62019974dd1704c7c9124def6`/i, "PROJECT_STATE must identify the exact Phase C first-slice merge.");
...
A.match(state, /Private Remote Joining[\s\S]{0,320}PRIORITIZED LONG-TERM[\s\S]{0,320}DEPENDENCY-GATED \/ NOT YET IMPLEMENTATION-AUTHORIZED/i, "PROJECT_STATE must preserve Remote Joining as prioritized long-term while retaining prerequisite gates.");
A.match(state, /Stage 1 Cloud \/ Sync Readiness Phase 1A through 1F is DONE \/ MERGED \/ PROTECTED/i, "PROJECT_STATE must record completed current Stage 1 authority.");
A.match(state, /Private Account \/ Authentication \/ Authorization Stages 2A through 2I are DONE \/ MERGED \/ PROVEN/i, "PROJECT_STATE must record completed Stage 2A-2I prerequisite boundaries.");
A.match(state, /Phase 1D — exact provider-compatible remote schema and API\/authorization contract:\s*\*\*DONE \/ MERGED \/ PROTECTED\*\*[\s\S]+Phase 1E — deterministic two-device\/offline\/reconnect synchronization harness:\s*\*\*CURRENT BOUNDED CANDIDATE\*\*[\s\S]+Phase 1F — \*\*\*NEXT AFTER PHASE 1E MERGES \/ BLOCKED\*\*\*/i, "PROJECT_STATE must retain the historical Phase 1D → 1E → 1F sequence explicitly as provenance.");

A.match(next, /CURRENT IMPLEMENTATION AUTHORITY — PR #125 SPARK PRIVATE CONNECTED ACCOUNT RUNTIME/i, "NEXT_TASK must identify PR #125 as current implementation authority.");
A.match(next, /Authorized product candidate:[\s\S]{0,180}v1\.5\.0[\s\S]{0,180}1\.5\.0-r1/i, "NEXT_TASK must identify the active v1.5 candidate.");
A.match(next, /Current production Installable Offline App runtime: `1\.4\.0-r2`[\s\S]{0,220}Immediate candidate rollback\/recovery runtime: `1\.4\.0-r2`/i, "NEXT_TASK must keep production r2 as the candidate recovery target.");
A.match(next, /Phase B first slice — Save Library \/ Local Profile Experience 2\.0 \(PR #70/i, "NEXT_TASK must retain Phase B first slice as closed evidence.");
A.match(next, /Phase C first slice — Showdown Home & Season Experience deepening \(PR #73/i, "NEXT_TASK must retain Phase C first slice as closed evidence.");
A.match(next, /65b6c9db0a070b6e5e992a39dffeee23df0c6f08/i, "NEXT_TASK must retain the PR #70 feature-merge SHA.");
A.match(next, /dec1d3ba8182c3f62019974dd1704c7c9124def6/i, "NEXT_TASK must retain the PR #73 feature-merge SHA.");
A.match(next, /IMMEDIATE NEXT TASK AFTER FULL STUDY/i, "NEXT_TASK must retain the permanent concrete handoff section.");
A.match(next, /formatVersion 2 full multi-Save backup\/import portability \(PR #67\)/i, "NEXT_TASK must retain multi-Save portability as closed evidence.");
A.match(next, /8fc671fc644e69b4fd405d7ebc28f961b2f3ae27/i, "NEXT_TASK must retain the PR #67 feature-merge SHA.");
A.match(next, /Local Profile display-label editing[\s\S]+Identity-Safe Career Analytics[\s\S]+formatVersion 2 full multi-Save/i, "NEXT_TASK must retain the closed local identity/analytics/portability chain.");
A.match(next, /Stage 1 Cloud \/ Sync Readiness Phase 1A through 1F remains DONE \/ MERGED \/ PROTECTED/i, "NEXT_TASK must preserve completed Stage 1 Cloud/Sync authority.");
...
A.match(next, /App Check enforcement:\s*OFF|App Check enforcement remains OFF/i, "NEXT_TASK must keep App Check enforcement off.");
A.match(next, /firestore\.spark\.rules[\s\S]+Do not publish production Firestore Rules until the exact candidate source head is clean/i, "NEXT_TASK must preserve the source-before-rules publication gate.");

A.match(historicalNext, /Cloud\/Sync Readiness Phase 1D merge: `fc2e8e8b921a435103a438a9239efbb890584d22`[\s\S]+No product candidate is currently authorized[\s\S]+Current authorized prerequisite candidate[\s\S]+Cloud\/Sync Readiness Phase 1E[\s\S]+Next prerequisite after Phase 1E merges[\s\S]+Cloud\/Sync Readiness Phase 1F/i, "Archived NEXT_TASK must retain the historical Phase 1D → Phase 1E → Phase 1F authorization sequence.");

A.match(roadmap, /Current verified source and `NEXT_TASK\.md` override every retained roadmap body below/i, "Roadmap must explicitly subordinate retained historical bodies to current source authority.");
A.match(roadmap, /Cloud Readiness \| PHASE 1A DONE \/ 1B DONE \/ 1C DONE \/ 1D DONE \/ 1E CURRENT \/ 1F NEXT/i, "Roadmap must retain the historical staged Cloud Readiness progression as provenance.");
A.match(roadmap, /Public\/community\/rankings \| ELIMINATED/i, "Roadmap must keep public community and global rankings eliminated.");
A.match(roadmap, /Private Remote Joining \| PRIORITIZED LONG-TERM \/ DEPENDENCY-GATED \/ NOT YET AUTHORIZED/i, "Roadmap must preserve Private Remote Joining as the prioritized strategic destination while retaining strict gates.");
...
A.match(phase1e, /Phase 1F remains blocked/i, "Phase 1E document must preserve its historical blocked-next gate.");
...
A.match(phase1eTest, /Object\.isFrozen\(a1\.intent\.content\)/, "Phase 1E permanent test must prove payload immutability.");
...
A.match(analyticsHandoff, /Failure 7[\s\S]+transient\/offscreen rendered-text assertion issue/i, "Closed Analytics handoff must preserve the final test classification.");
...
process.stdout.write(`PASS release authority coherence for v${version}/${revision}; candidate-versus-production identity, archived Cloud/Sync provenance, recovery semantics, completed product layers, Remote Joining locks and workflow topology agree.\n`);