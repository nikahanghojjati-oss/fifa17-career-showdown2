...
if(candidateRecord && currentProductionProven){
    ...
    A.match(currentNext, /IMMEDIATE NEXT TASK AFTER FULL STUDY[\s\S]+mandatory recursive SLE package[\s\S]+publish/i, "NEXT_TASK must route forward to the sealed transition publication rather than revive a completed product proof.");
