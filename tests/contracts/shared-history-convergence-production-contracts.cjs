...
process.stdout.write('PASS Shared History Convergence production contracts: r12 remains dormant until exact cached r10 ACKNOWLEDGED and r11 SCORING_RECONCILED authority exists for the same season, refreshes those predecessors before one exact read-only provider projection, renders converged season history/manager records/trophies plus cumulative Showdown totals and lead without a second scoring policy, preserves stable account/device/rivalry/session identity, and exposes no write/list/billing authority.\n');
