...
assert.match(appCss,/R9 SLICE 3 — SHARED DASHBOARD \/ RECONNECT PRESENTATION/,'R9 Shared Dashboard visual layer is missing.');
assert.match(appCss,/#dashboard\[data-shared-dashboard-authority="true"\] \.showdownScoreboard/,'R9 must style the real authoritative aggregate scoreboard.');
assert.match(appCss,/#dashboard #seasonPrimaryAction\[data-shared-transfer-challenge="true"\]/,'R9 must style the real Shared Transfer primary action rather than create a duplicate Dashboard CTA.');
