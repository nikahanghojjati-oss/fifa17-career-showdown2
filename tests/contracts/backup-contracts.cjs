...
(async () => {
    ...
    assert.ok(backupSource.includes("captureCareerModeRawRestoreSnapshot"), "Candidate A must use strict exact raw snapshot authority rather than a lossy canonical reader.");
    ...
    await assert.rejects(
        ...
        "A blocked canonical read must fail Candidate A closed rather than exporting an apparently valid incomplete backup."
    );
    assert.equal(runtime.counters.set, blockedBefore.set, "Blocked-read backup failure must not write localStorage.");
    assert.equal(runtime.counters.remove, blockedBefore.remove, "Blocked-read backup failure must not remove localStorage data.");
