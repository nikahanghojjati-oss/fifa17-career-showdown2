...
const save1="save_"+("1".repeat(24)),save2="save_"+("2".repeat(24));
const profile1="profile_"+("3".repeat(24)),profile2="profile_"+("4".repeat(24));
...
async function harness({expiresAt=9_000_000}={}){
  const key=(...parts)=>parts.join("/"),store=new Map();
...
(async()=>{
  ...
  assert.equal(accepted.ok,true);assert.equal(accepted.status,"accepted");assert.equal(accepted.replayed,false);assert.equal(accepted.rivalryState,"closed");assert.equal(accepted.sessionState,"closed");assert.equal(accepted.rivalryRevision,3);assert.equal(accepted.sessionRevision,6);
  const rivalry=h.store.get(h.key("rivalries",rivalryId)),session=h.store.get(h.key("rivalries",rivalryId,"sessions",sessionId));
  assert.equal(rivalry.data.connectionState,"closed");assert.equal(Terminal.sameWitness(rivalry.data.terminalClose,intent),true);assert.equal(session.data.state,"closed");assert.deepEqual(rivalry.data.authorizedAccountIds,[uid1,uid2]);assert.equal(rivalry.data.managerSlots.every(slot=>slot.entitlementState==="active"),true);

  const replay=await Provider.close(h.options(2));assert.equal(replay.ok,true);assert.equal(replay.replayed,true);assert.equal(replay.rivalryRevision,3);assert.equal(replay.sessionRevision,6);
  const terminalRead=await Provider.read({user:{uid:uid2},firestore:{},firebaseSdk:h.sdk,rivalryId,deviceId:device2,cryptoImpl:webcrypto});assert.equal(terminalRead.ok,true);assert.equal(terminalRead.terminal,true);assert.equal(terminalRead.rivalryState,"closed");assert.equal(Terminal.sameWitness(terminalRead.terminalWitness,intent),true);
  ...
  assert.equal(newSession.ok,false);assert.equal(newSession.code,"PRIVATE_SESSION_RIVALRY_INACTIVE");assert.equal(h.store.has(h.key("rivalries",rivalryId,"sessions",freshSessionId)),false);

  const changed=JSON.parse(JSON.stringify(intent));changed.managerTotals.playerOne=12;changed.winner="playerOne";
  const conflict=await Provider.close(h.options(1,{intent:changed}));assert.equal(conflict.ok,false);assert.equal(conflict.code,"TERMINAL_CLOSE_REPLAY_CONFLICT");
  assert.equal(h.store.get(h.key("rivalries",rivalryId)).revision,3);assert.equal(h.store.get(h.key("rivalries",rivalryId,"sessions",sessionId)).revision,6);

  const expired=await harness({expiresAt:1_500_000});const denied=await Provider.close(expired.options(1));assert.equal(denied.ok,false);assert.equal(denied.code,"TERMINAL_CLOSE_SESSION_EXPIRED");assert.equal(expired.store.get(expired.key("rivalries",rivalryId)).data.connectionState,"active");assert.equal(expired.store.get(expired.key("rivalries",rivalryId,"sessions",sessionId)).data.state,"active");

  const revoked=await harness();revoked.store.get(revoked.key("accounts",uid1,"devices",device1)).data.state="revoked";const revokedResult=await Provider.close(revoked.options(1));assert.equal(revokedResult.ok,false);assert.equal(revokedResult.code,"TERMINAL_CLOSE_DEVICE_REVOKED");assert.equal(revoked.store.get(revoked.key("rivalries",rivalryId)).data.connectionState,"active");

  console.log("PASS Terminal Close provider: atomic rivalry+session closure, exact witness replay, cross-manager terminal read, no fresh-session resurrection, expiry/device fail-closed behavior, no new collection, zero billing and no canonical local-save mutation.");
