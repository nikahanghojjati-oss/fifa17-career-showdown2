...
assert.match(next, /active bounded candidate is:[\s\S]+Local Profile display-label editing/i, "NEXT_TASK must identify the one local-only candidate without turning completed Analytics into implicit Cloud authorization.");
assert.match(next, /Cloud Readiness[\s\S]+None beyond the exact display-label candidate above is implementation-authorized/i, "NEXT_TASK must keep Cloud Readiness and later network work outside the current authorization boundary.");
...
process.stdout.write("PASS future cloud contract preserves shipped local identity and Identity-Safe Career Analytics, unresolved-history honesty, recovery authority and cloud security/revision boundaries while the sole active label candidate remains local-only\n");
