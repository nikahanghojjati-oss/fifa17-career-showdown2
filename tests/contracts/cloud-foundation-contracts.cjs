...
assert.match(state, /No product candidate is currently authorized for implementation|formatVersion 2 is live/i, "PROJECT_STATE must retain multi-Save closed state and no open product candidate.");
...
assert.match(next, /No product candidate is currently authorized/i, "NEXT_TASK must state that no product candidate is currently authorized after Phase B first slice proof.");
