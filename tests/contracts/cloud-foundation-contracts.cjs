...
assert.match(next, /No new substantial runtime product candidate is authorized/i, "NEXT_TASK must not turn completed Analytics into implicit Cloud authorization.");
assert.match(next, /Cloud Readiness[\s\S]+None is implementation-authorized/i, "NEXT_TASK must keep Cloud Readiness and later network work outside the current authorization boundary.");
...
process.stdout.write("PASS future cloud contract preserves shipped local identity and Identity-Safe Career Analytics, unresolved-history honesty, recovery authority and cloud security/revision boundaries while no cloud runtime is authorized\n");
