...
assert.match(roadmap, /Cloud Readiness \| PHASE 1A DONE \/ 1B DONE \/ 1C CURRENT \/ 1D NEXT/i, "Roadmap must advance the active Cloud Readiness lane through the current Phase 1C boundary while keeping runtime gated.");
...
assert.match(state, /Phase 1C[\s\S]+CURRENT BOUNDED CANDIDATE/i, "PROJECT_STATE must identify the active Phase 1C architecture candidate.");
...
assert.match(next, /Current authorized prerequisite candidate[\s\S]+Cloud\/Sync Readiness Phase 1C/i, "NEXT_TASK must keep Phase 1C as the bounded current architecture prerequisite.");
assert.match(next, /Next prerequisite after Phase 1C merges[\s\S]+Cloud\/Sync Readiness Phase 1D/i, "NEXT_TASK must advance exactly to Phase 1D after Phase 1C proves clean.");
...
assert.match(remoteRoadmap, /Phase 1A[\s\S]+DONE \/ MERGED \/ PROTECTED[\s\S]+Phase 1B[\s\S]+DONE \/ MERGED \/ PROTECTED[\s\S]+Phase 1C[\s\S]+CURRENT BOUNDED CANDIDATE[\s\S]+Phase 1D[\s\S]+NEXT AFTER PHASE 1C MERGES/i, "Detailed Remote Joining roadmap must reflect exact staged Cloud Readiness progression through Phase 1C.");
...
process.stdout.write("PASS Cloud/Sync Readiness authority: Phase 1A and 1B proven, Phase 1C current, Phase 1D next, runtime gated, recovery/privacy and Remote Joining dependency locks preserved\n");