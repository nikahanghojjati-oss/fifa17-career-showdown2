...
const forbidden=["NEXT_TASK.md","00_CURRENT_HANDOFF.md","WORK_ENVIRONMENT_STATUS.json","WORK_ENVIRONMENT_ARCHIVE","START_NEXT_SESSION","SUCCESSOR_HANDOFF"];
...
assert.deepEqual(offenders,[],`GP-2 current product gates must not import process/continuity authority:\n${offenders.join("\n")}`);
...
assert.equal(dependencyPattern("NEXT_TASK.md").test(executableSource(`const next=read("NEXT_TASK.md");`)),true,"GP-2 must detect real process dependencies.");
assert.ok(!forbidden.includes("REMOTE_JOINING_READINESS.json"),"GP-2 must not ban machine product-readiness dependencies; SSJR-1.1 legitimately verifies completed RJR-1 before credit.");
