...
assert.match(nextTask,/^# CURRENT OVERRIDE — PR #163[\s\S]+PRODUCTION-PROVEN[\s\S]+RJR84[\s\S]+SLE TRANSITION/im,"Live NEXT_TASK must identify the current PR #163 production/SLE transition authority rather than revive PR #125, the r3 recovery incident, or the pre-merge publication heading.");
...
process.stdout.write("PASS trusted connected data account export boundary: private explicit portability, exact entitlement/read scope, peer-identity minimization, secret exclusion, dormant trusted-export isolation and unchanged IAM/browser-write locks remain protected under current PR #163 production authority.\n");
