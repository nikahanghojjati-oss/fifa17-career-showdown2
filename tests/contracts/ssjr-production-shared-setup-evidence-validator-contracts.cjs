...
function bundle(managerRole,remoteRole,account,device){
  ...
  return {schemaVersion:1,evidenceType:'SSJR-1.1-production-shared-setup',capturedAt:'2026-09-07T09:30:00Z',runtimeRevision:'1.9.1-r5',managerRole,remoteRole,accountFingerprint:hash(account),deviceFingerprint:hash(device),rivalryFingerprint:hash('rivalry-a'),canonicalStorageBeforeHash:storage,canonicalStorageAfterHash:storage,checkpoints:[
    {name:'paired-active-before-setup',at:'2026-09-07T09:30:01Z',paired:true,sessionState:'active',setupMutationSeen:false,sessionFingerprint:session},
    {name:'authoritative-setup-observed',at:'2026-09-07T09:30:02Z',revision:1,setupDigest:seed},
    {name:'identical-final-setup',at:'2026-09-07T09:30:03Z',setupDigest:setup.digest},
    {name:'reload-resume',at:'2026-09-07T09:30:04Z',setupDigest:setup.digest,resetOrRedraw:false},
    {name:'fresh-active-session-resume',at:'2026-09-07T09:30:05Z',setupDigest:setup.digest,resetOrRedraw:false,sessionFingerprint:fresh}
...
let result=run(source,peer);assert.equal(result.status,0,result.stderr);const summary=JSON.parse(result.stdout);assert.equal(summary.ok,true);assert.equal(summary.runtimeRevision,'1.9.1-r5');assert.equal(summary.finalRevision,6);assert.equal(summary.finalPhase,'SHOWDOWN_CONFIRMED');assert.equal(summary.authoritativeCatalogValidated,true);assert.equal(summary.rawAuthorityRejected,true);assert.equal(summary.canonicalStoragePreserved,true);assert.equal(summary.negativesProvenPerManager,8);
for(const [label,mutate,needle] of [
  ...
  ['obsolete runtime',(a,b)=>{a.runtimeRevision='1.9.1-r4';b.runtimeRevision='1.9.1-r4';},'exact deployed production runtime 1.9.1-r5'],
  ['older runtime',(a,b)=>{a.runtimeRevision='1.9.1-r3';b.runtimeRevision='1.9.1-r3';},'exact deployed production runtime 1.9.1-r5'],
  ['impossible runtime',(a,b)=>{a.runtimeRevision='1.9.1-r999';b.runtimeRevision='1.9.1-r999';},'exact deployed production runtime 1.9.1-r5'],
...
console.log('PASS SSJR production Shared Setup evidence validator contracts: exact r5, schema-closed privacy, catalog membership and final revision 6 are enforced.');