...
let result=run(source,peer);assert.equal(result.status,0,result.stderr);const summary=JSON.parse(result.stdout);assert.equal(summary.ok,true);assert.equal(summary.runtimeRevision,'1.9.1-r4');assert.equal(summary.finalRevision,6);assert.equal(summary.finalPhase,'SHOWDOWN_CONFIRMED');assert.equal(summary.authoritativeCatalogValidated,true);assert.equal(summary.rawAuthorityRejected,true);assert.equal(summary.canonicalStoragePreserved,true);assert.equal(summary.negativesProvenPerManager,8);
for(const [label,mutate,needle] of [
  ...
  ['obsolete runtime',(a,b)=>{a.runtimeRevision='1.9.1-r3';b.runtimeRevision='1.9.1-r3';},'exact deployed production runtime 1.9.1-r4'],
  ['impossible runtime',(a,b)=>{a.runtimeRevision='1.9.1-r999';b.runtimeRevision='1.9.1-r999';},'exact deployed production runtime 1.9.1-r4'],
...
console.log('PASS SSJR production Shared Setup evidence validator contracts: exact r4, schema-closed privacy, catalog membership and final revision 6 are enforced.');