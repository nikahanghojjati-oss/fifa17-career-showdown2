...
assert.ok(source.storage.includes("window.getCareerModeStorageKeys=()=>({activeShowdown:STORAGE_KEY,legacyShowdowns:LEGACY_STORAGE_KEY,preferences:APPLICATION_PREFERENCES_KEY})"), "The public current storage-key contract must remain the existing three canonical keys until runtime cutover.");
