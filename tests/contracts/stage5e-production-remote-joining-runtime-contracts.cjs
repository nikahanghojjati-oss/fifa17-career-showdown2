...
assert.equal(html.includes('id="remoteJoiningButton"'),false,"Remote Joining must not be a separate player-facing dashboard action.");
...
assert.equal(api.openPanel(),false,"No-document open must remain inert.");
assert.equal(calls.services,0,"Opening UI without an action must not initialize Firebase services.");
