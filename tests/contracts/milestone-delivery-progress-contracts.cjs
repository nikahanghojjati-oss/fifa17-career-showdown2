...
(async () => {
  ...
  assert.equal(result.score, 93.7, "r17 Final Reconciliation earns product integration only after exact publication-head validation, expected-head merge, coherent Pages deployment, main POS20 validation and two-pass release burn-in.");
  assert.equal(result.formattedScore, "93.70/100");
  assert.equal(result.featureCount, 20);
  assert.equal(result.fullyLifecycleDelivered, 17);
  assert.equal(result.preIntegrationComplete, 0);
  assert.equal(result.designDefinedOnly, 3);

  assert.equal(ledger.basis.mainSha, "23bba67ed7bf9cc38cb7dc9106e3f64a17a6b55f");
  assert.equal(ledger.basis.branch, "main");
  assert.equal(ledger.basis.integrationMergeSha, "23bba67ed7bf9cc38cb7dc9106e3f64a17a6b55f");
  assert.equal(ledger.basis.candidateHead, "054b57be3cf398ff4c812f8c6f813e4970c6bded");
  assert.match(ledger.basis.candidateValidation, /POS20 run #430 exact-head green/);
  assert.equal(ledger.basis.productionRuntimeRevision, "1.9.1-r17");
  assert.match(ledger.basis.pagesDeployment, /GitHub Pages run #124 success/i);
  assert.match(ledger.basis.firestoreRulesDeployment, /No new Firestore Rules deployment required/i);
  assert.match(ledger.basis.note, /POS20 run #431 passed/i);
  assert.match(ledger.basis.note, /Release Integration Burn-In run #381 passed both/i);
  ...
  const integrated = new Set([
    ...
    "career-start","transfer-challenge","results-publication","season-commit","canonical-scoring","history-convergence","multi-season","journey-reconnect","journey-conflicts","local-reconciliation","final-reconciliation"
  ]);
  ...
  assert.equal(ledger.scoreCheck.fullyIntegratedWeight, 93);
  assert.equal(ledger.scoreCheck.remainingDesignWeight, 7);
  assert.equal(ledger.scoreCheck.remainingDesignContribution, 0.7);
  assert.equal(ledger.scoreCheck.calculatedScore, 93.7);
  ...
  const outOfOrder = clone(ledger); const terminalClose = outOfOrder.capabilities.find(capability => capability.id === "terminal-close"); terminalClose.stages["automated-test"] = "complete";
  ...
  process.stdout.write(`PASS MDP-1 exact SSJR denominator, six-stage lifecycle, current ${result.formattedScore} r17 integrated ledger, anti-inflation rules, regression guard and strict SSJR separation\n`);
