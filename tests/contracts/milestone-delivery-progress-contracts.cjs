...
(async () => {
  ...
  assert.equal(result.score, 95.5, "r18 Terminal Close earns product integration only after exact publication-head validation, expected-head merge, coherent Pages and production Rules publication/readback, main POS20 validation and two-pass release burn-in.");
  assert.equal(result.formattedScore, "95.50/100");
  assert.equal(result.featureCount, 20);
  assert.equal(result.fullyLifecycleDelivered, 18);
  assert.equal(result.preIntegrationComplete, 0);
  assert.equal(result.designDefinedOnly, 2);

  assert.equal(ledger.basis.mainSha, "457281e672008e02a5e7e893a13a7f77a60f0162");
  assert.equal(ledger.basis.branch, "main");
  assert.equal(ledger.basis.integrationMergeSha, "457281e672008e02a5e7e893a13a7f77a60f0162");
  assert.equal(ledger.basis.candidateHead, "73533cbe059b831165f51927ae2240303b2f5da6");
  assert.match(ledger.basis.candidateValidation, /POS20 run #436 exact-head green/);
  assert.equal(ledger.basis.productionRuntimeRevision, "1.9.1-r18");
  assert.match(ledger.basis.pagesDeployment, /GitHub Pages run #129 success/i);
  assert.match(ledger.basis.firestoreRulesDeployment, /Deploy Firebase Firestore Rules - Zero Billing run #11 success/i);
  assert.match(ledger.basis.note, /POS20 #449 passed/i);
  assert.match(ledger.basis.note, /Release Integration Burn-In #386 passed both/i);
  ...
  const r17Candidate = ssjrReadiness.candidateEvidence.find(item => item.id === "ssjr1-final-reconciliation-r17-production");
  assert.ok(r17Candidate, "SSJR readiness must preserve truthful zero-credit r17 production evidence.");
  assert.equal(r17Candidate.credit, 0);
  assert.deepEqual(r17Candidate.relatedCapabilityIds, ["final-reconciliation"]);
  for (const layer of ["deterministic-behavior","provider-enforcement","isolated-browser-protocol","deployed-runtime"]) assert.ok(r17Candidate.layers.includes(layer), `r17 candidate evidence missing ${layer}`);
  assert.ok(r17Candidate.missingLayers.includes("production-two-account"));
  for (const expected of ["PR #248 exact publication head 054b57be","POS20 run #430 exact-head green","PR #248 merge 23bba67e","GitHub Pages run #124 success","POS20 run #431 main green","Release Integration Burn-In run #381 success"]) assert.ok(r17Candidate.references.some(ref => ref.includes(expected)), `r17 evidence missing ${expected}`);
  assert.equal(ssjrReadiness.remainingCapabilityIds.includes("final-reconciliation"), true, "Final Reconciliation remains uncredited in SSJR until production-two-account evidence is accepted.");

  const r18Candidate = ssjrReadiness.candidateEvidence.find(item => item.id === "ssjr1-terminal-close-r18-production");
  ...
  const integrated = new Set([
    ...
    "career-start","transfer-challenge","results-publication","season-commit","canonical-scoring","history-convergence","multi-season","journey-reconnect","journey-conflicts","local-reconciliation","final-reconciliation","terminal-close"
  ]);
  ...
  const finalReconciliationIntegrated = ledger.capabilities.find(capability => capability.id === "final-reconciliation");
  assert.equal(finalReconciliationIntegrated.weightedContribution, 3);
  for (const ref of ["js/sharedFinalReconciliation.js","js/productionSharedFinalReconciliation.js","tests/contracts/shared-final-reconciliation-production-contracts.cjs","tests/browser/shared-final-reconciliation-audit.cjs"]) assert.ok(finalReconciliationIntegrated.evidenceRefs.includes(ref), `Final Reconciliation evidence missing ${ref}`);
  for (const expected of ["PR #248 exact publication head 054b57be","POS20 run #430 exact-head green","PR #248 merge 23bba67e","GitHub Pages run #124 success","POS20 run #431 main green","Release Integration Burn-In run #381 success"]) assert.ok(finalReconciliationIntegrated.evidenceRefs.some(ref => ref.includes(expected)), `Final Reconciliation integration evidence missing ${expected}`);

  const terminalCloseIntegrated = ledger.capabilities.find(capability => capability.id === "terminal-close");
  ...
  assert.equal(ledger.scoreCheck.fullyIntegratedWeight, 95);
  assert.equal(ledger.scoreCheck.remainingDesignWeight, 5);
  assert.equal(ledger.scoreCheck.remainingDesignContribution, 0.5);
  assert.equal(ledger.scoreCheck.calculatedScore, 95.5);
  ...
  const outOfOrder = clone(ledger); const physicalJourney = outOfOrder.capabilities.find(capability => capability.id === "physical-journey"); physicalJourney.stages["automated-test"] = "complete";
  ...
  process.stdout.write(`PASS MDP-1 exact SSJR denominator, six-stage lifecycle, current ${result.formattedScore} r18 integrated ledger, anti-inflation rules, regression guard and strict SSJR separation\n`);
