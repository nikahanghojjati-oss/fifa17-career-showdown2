...
assert.ok(html.includes(`v${appVersion} · Stable`) || html.includes(`v${appVersion} · Product Deepening`) || html.includes(`v${appVersion} · Private Connected Account Foundation`) || html.includes(`v${appVersion} · Registered Devices & Private Pairing`) || html.includes(`v${appVersion} · Connected Rivalry`) || html.includes(`v${appVersion} · Private Remote Joining`), 'Footer release identity must track APP_VERSION.');
