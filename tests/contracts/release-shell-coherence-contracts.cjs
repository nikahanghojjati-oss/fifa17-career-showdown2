...
const html=read("index.html"),app=read("js/app.js"),worker=read("service-worker.js"),manifest=read("manifest.webmanifest"),release=read("RELEASE_V1.9.0.md");
const runtime=(html.match(/app-asset-revision" content="([^"]+)"/)||[])[1];
const previous=(worker.match(/const PREVIOUS_RUNTIME_REVISION = "([^"]+)";/)||[])[1];
assert.equal(runtime,"1.9.0-r1");
