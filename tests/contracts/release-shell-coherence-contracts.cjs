...
const html=read("index.html"),app=read("js/app.js"),worker=read("service-worker.js"),manifest=read("manifest.webmanifest"),release=read("RELEASE_V1.8.1_R2.md");
assert.match(app,/const APP_VERSION = "1\.8\.1";/);
assert.match(html,/app-asset-revision" content="1\.8\.1-r2"/);
assert.match(worker,/const RUNTIME_REVISION = "1\.8\.1-r2";/);
assert.match(worker,/const PREVIOUS_RUNTIME_REVISION = "1\.8\.1-r1";/);
...
assert.match(release,/Runtime asset revision: `1\.8\.1-r2`/);
assert.match(release,/Previous known-good runtime: `1\.8\.1-r1`/);
assert.match(manifest,/1\.8\.1-r2/);
