...
for(const text of [prompt,checked]){
 assert.match(text,/START_NEXT_SESSION_V1\.4\.54_PR210_POSTMERGE_GREEN_SSJR_PRIVATE_TWO_ACCOUNT_NEXT\.md/);
...
process.stdout.write("PASS next-developer prompt: compact v1.4.54 repository-first SNS matches its generator and routes a fresh WEC through current live authority to the detailed PR210 successor package.\n");