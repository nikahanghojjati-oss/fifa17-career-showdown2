...
assert.match(checkedPrompt,/PR #187/i);
assert.match(checkedPrompt,/v1\.9\.0 \/ 1\.9\.0-r5/i);
assert.match(checkedPrompt,/89\/100/i);
assert.match(checkedPrompt,/fresh unique WEC/i);
assert.match(checkedPrompt,/smallest genuinely uncredited/i);
assert.match(checkedPrompt,/Billing must never be activated/i);

process.stdout.write("PASS next-developer prompt: v1.4.37 mirrored package and repository-first generated/current prompts route a fresh successor from PR187 r5 RJR89.\n");
