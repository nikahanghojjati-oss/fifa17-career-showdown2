...
assert.match(next,/Historical heading: CURRENT IMPLEMENTATION AUTHORITY — PRODUCTION APP CHECK RUNTIME INTEGRATION/i,"Current authority must retain the former App Check-runtime heading only as provenance.");
...
assert.match(state,/Private Account \/ Authentication \/ Authorization Stages 2A through 2I are DONE \/ MERGED \/ PROVEN/i,"PROJECT_STATE must preserve the completed Stage 2A-through-2I prerequisite boundary instead of restoring obsolete dormant-stage wording.");
...
process.stdout.write("PASS Private Account/Auth Stage 2E trusted application-account bootstrap with historical Stage 2F/2G transition preserved and current PR #125/v1.5 candidate authority explicit\n");