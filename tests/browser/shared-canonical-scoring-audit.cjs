...
async function prepare(page,{role,saveId}){
  ...
  await page.evaluate(async()=>{
    ...
    if(!window.CareerModeSharedCanonicalScoring)await bounded(loadCandidateScript('js/sharedCanonicalScoring.js'),'candidate scoring core fallback load');
    if(!window.CareerModeProductionSharedCanonicalScoring)await bounded(loadCandidateScript('js/productionSharedCanonicalScoring.js'),'candidate production scoring fallback load');
    ...
    if(!refreshed||!state||!panel||panel.classList.contains('hidden')){
      ...
      throw new Error(`r11 scoring refresh/render mismatch refreshed=${Boolean(refreshed)} state=${JSON.stringify(state)} commit=${JSON.stringify(commitState)} panel=${panel?panel.className:'missing'} providerCalls=${JSON.stringify(window.__ssjrScoringProviderCalls)}`);
    }
    const base=window.__ssjrScoringAuditBase;
    window.__ssjrScoringAudit={providerCalls:window.__ssjrScoringProviderCalls,storageBefore:base.storageBefore,storageAfter:base.storageAfter,localState:base.localState,state:()=>CareerModeProductionSharedCanonicalScoring.getState()};
  });
...
(async()=>{
  ...
  try{
    ...
    process.stdout.write('PASS Shared Canonical Scoring desktop/mobile production flow: both managers traverse the real r9 results and r10 commit adapters, the bootstrapped r11 scoring adapter converges without duplicate observers, both see the same provider-authoritative 11–0 canonical score and five-part breakdown only after r10 ACKNOWLEDGED, exact account/device/rivalry/session context is forwarded for provider enforcement, and canonical local storage/history/scoring remain untouched.\n');
