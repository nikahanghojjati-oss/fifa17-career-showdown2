...
(async()=>{
  const runtime=await resolveChromiumRuntime();let normal=null,integrity=null,candidate=null,chronology=null,physical=null;
  try{
    ...
    await chronology.context.setOffline(true);await chronology.page.waitForTimeout(100);await chronology.context.setOffline(false);await chronology.page.waitForTimeout(100);await installFixture(chronology.page,"recovered");await chronology.page.evaluate(()=>window.CareerModeSSJRPhysicalJourneyAcceptance.observe());await chronology.page.waitForFunction(()=>window.CareerModeSSJRPhysicalJourneyAcceptance.getState().onlineRecovered===true,null,{timeout:3000});
    const afterRecoverySameStartup=await chronology.page.evaluate(()=>window.CareerModeSSJRPhysicalJourneyAcceptance.getEvidence());assert.equal(afterRecoverySameStartup.reloadResumed,false,"a startup that predates recovery must not count as the required recovery reload");const recoveredMilestone=afterRecoverySameStartup.milestones.find(item=>item.stage==="reconnect-recovered");assert.equal(recoveredMilestone.startupCount,2);
    ...
    console.log("PASS Physical Journey ignores pre-History network noise and requires a startup strictly after reconnect for reload-resumed");
  ...
  }finally{await closeCase(physical);await closeCase(chronology);await closeCase(candidate);await closeCase(integrity);await closeCase(normal);}
