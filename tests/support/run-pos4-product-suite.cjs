...
for (const file of files) {
  ...
  if (result.status !== 0 || result.error) {
    failures.push({ file, status: result.status, signal: result.signal, error: result.error?.message || null });
  }
}

if (failures.length) {
  console.error(`FC-1 repository census failed in ${failures.length} suite entrypoint(s):`);
  for (const failure of failures) {
    console.error(`- ${failure.file}${failure.signal ? ` signal=${failure.signal}` : ""}${failure.error ? ` error=${failure.error}` : ""}`);
  }
  process.exitCode = 1;
} else {
  console.log(`PASS FC-1 repository current-product census (${files.length} entrypoints)`);
}
