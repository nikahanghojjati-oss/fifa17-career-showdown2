...
for (const file of manifest.tests) {
  if (seen.has(file)) {
    failures.push({ file, detail: "Duplicate current product test." });
    continue;
  }
  seen.add(file);
  if (!fs.existsSync(file)) {
    failures.push({ file, detail: "Current product test is missing." });
    continue;
  }
...
if (failures.length) {
  console.error(`FC-1 FAILURE CENSUS: ${failures.length} of ${manifest.tests.length} current product contracts failed.`);
  for (const failure of failures) console.error(`- ${failure.file}`);
  process.exitCode = 1;
} else {
  process.stdout.write(`PASS FC-1 current product contract census (${manifest.tests.length} tests, 0 failures)\n`);
}
