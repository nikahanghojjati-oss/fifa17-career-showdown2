...
printf '\n=== Candidate C release burn-in pass %s: static/contracts ===\n' "$pass"
find js data -type f -name '*.js' -print0 | xargs -0 -n1 node --check
npm run test:contracts

printf '\n=== Candidate C release burn-in pass %s: browser server ===\n' "$pass"
...
printf '\n=== pass %s: Candidate C atomic restore/recovery ===\n' "$pass"
npm run test:restore-browser
printf '\n=== pass %s: complete journey ===\n' "$pass"
npm run test:browser

printf '\n=== Candidate C release burn-in pass %s PASSED ===\n' "$pass"
