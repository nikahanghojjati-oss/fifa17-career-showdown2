...
const files=[
  'tests/contracts/project-operating-system-v7-contracts.cjs',
  'tests/contracts/pos7-continuity-contracts.cjs',
...
if(failures.length){
  console.error(`POS7 operations census failed in ${failures.length} file(s):`);
  for(const failure of failures)console.error(`- ${failure.file}: ${failure.error}`);
  process.exitCode=1;
}else console.log(`PASS POS7 operations audit (${files.length} files)`);
