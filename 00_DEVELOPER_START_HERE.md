...
Use the connected GitHub app first. Run the repository-owned GitHub CLI bootstrap only for a concrete connector gap; connector credentials must never be extracted or repurposed. SLE = Smart Lean Efficient remains recursive. At `Handoff proximity: 100%`, finish only the current safe checkpoint, generate/publish the complete mirrored SNS/SLE successor package, seal/archive the WEC and stop before another substantial milestone.
