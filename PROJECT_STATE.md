...
Owner visual acceptance is separate from automated technical proof and is not inferred here.

## Current milestone

Current milestone — v1.3.0 — Recovery & Device Resilience Hardening.

Implementation and technical production proof are closed. The current legal activity is release observation, preservation of the proven baseline and response to new evidence. Do not start a later feature milestone merely because v1.3 is technically closed; later scope still requires current owner/repository authority.
...
There are 14 permanent workflow families and 27 protected multiline executable blocks. Normal PRs run 13; Release Integration Burn-In is main/manual release-only.

The v1.3 production seal is Pages `31755135819`, Stability `31755136265`, deployed-site-smoke `94629478166` and Burn-In `31755136240` 2/2.

## Branch authority

PR #37 / `agent/v13-hardening` remains untrusted historical work and must not be merged or used as a baseline. PR #40 is the detailed salvage/audit record. PR #42 is the actual release PR.

Local Manager Profiles, Save Library registry redesign, cloud, accounts, synchronization, QR pairing, gameplay/scoring changes and framework migration remain future work and are not implicitly authorized by the v1.3 production seal.
