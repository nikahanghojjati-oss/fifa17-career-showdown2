...
At successor Handoff proximity 100%, generate the next complete mirrored recursive SLE package, seal its WEC as the final branch mutation, generate the permanent repository-first next-developer prompt with `npm run work:next-prompt`, provide that prompt to the owner, and stop before starting another substantial milestone.
