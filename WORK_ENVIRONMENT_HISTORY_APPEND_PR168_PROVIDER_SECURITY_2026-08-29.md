...
- Provider-path finding: root `firebase.json -> firestore.rules` and root `.firebaserc -> fifa17-career-showdown-demo` are intentional historical/emulator locks. Permanent Phase 1F and production-activation contracts protect them. They must not be redirected to `firestore.spark.rules`.
