...
function validateSingle(evidence,source,{expectedAppVersion,expectedRuntimeRevision}){
  ...
  require(evidence.canonicalStorageProofScope===STORAGE_SCOPE,"CANONICAL_STORAGE_PROOF_SCOPE_INVALID","Canonical storage integrity must be scoped to the read-only Local Reconciliation preview.");
  ...
  require(evidence.canonicalStorageViolation===false,"CANONICAL_STORAGE_CHANGED","Canonical local storage changed while the read-only Local Reconciliation preview was being observed.");
  require(evidence.canonicalStorageBeforeHash===evidence.canonicalStorageAfterHash,"CANONICAL_STORAGE_HASH_MISMATCH","Canonical local storage hashes must match immediately before and after the read-only Local Reconciliation preview.");
  ...
  require(Boolean(history&&offline&&online&&recovered&&reload&&storageBaseline&&storageVerified&&localPreview&&finalState&&terminal&&terminalReload&&history.sequence<offline.sequence&&offline.sequence<online.sequence&&online.sequence<recovered.sequence&&recovered.sequence<reload.sequence&&reload.sequence<storageBaseline.sequence&&storageBaseline.sequence<storageVerified.sequence&&storageVerified.sequence<localPreview.sequence&&localPreview.sequence<finalState.sequence&&finalState.sequence<terminal.sequence&&terminal.sequence<terminalReload.sequence),"RECOVERY_ORDER_INVALID","History, offline, online, reconnect, pre-terminal reload, scoped storage proof, Local Reconciliation preview, Final Reconciliation, terminal and terminal-reload milestones must occur in the canonical order.");
...
export function validatePhysicalJourneyPair(first,second,options={}){
  const expectedAppVersion=String(options.expectedAppVersion||"1.9.1"),expectedRuntimeRevision=String(options.expectedRuntimeRevision||"1.9.1-r20");
