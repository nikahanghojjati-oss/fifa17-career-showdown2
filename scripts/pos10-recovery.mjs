...
export function validateIndex(index) {
  ...
  for (const artifact of index.artifacts) {
    ...
    if (artifact.supersedes !== null) ensure(index.artifacts.some(a=>a.path===artifact.supersedes && a.role===artifact.role && a.status==='historical'), 'Broken supersession lineage');
  }
...
export function generatePackage(cwd, input) {
  ...
  const sequence = String((old?.artifacts.filter(a=>a.role==='successor-transfer').length || 0)+1).padStart(6,'0');
...
if(process.argv[1] && path.resolve(process.argv[1])===fileURLToPath(import.meta.url)) {
  try {
    ...
    if(command==='generate' && args.length===1) {
      ...
      const result=assessRecovery({...state,recoveryState:checkPackage(root).recoveryState});
