...
export const model = 'IMPACT-7';
const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const graphPath = path.join(root, 'POS7_IMPACT_GRAPH.json');
...
function expandInvariantClosure(seedIds){
  ...
  while(queue.length){
    ...
    if(!inv) throw new Error(`Unknown IMPACT-7 invariant: ${id}`);
...
function expandBundles(bundleIds){
  ...
  for(const bundle of bundleIds){
    ...
    if(!ids) throw new Error(`Unknown IMPACT-7 proof bundle: ${bundle}`);
    proofs.push(...ids);
  }
  return uniq(proofs);
}
...
export function routeFiles(input, options = {}){
  ...
  const nonDocs = files.filter(file => !match(file, graph.docsPatterns));
  const operationsOnly = nonDocs.length > 0 && nonDocs.every(file => match(file, graph.operationsPatterns));
  if(operationsOnly){
...
function validateAuthority(){
  ...
  if(manifest.operatingSystem !== 'POS-7') throw new Error('CURRENT_PRODUCT_TEST_MANIFEST.json must declare POS-7.');
  const ownedIndexes = Object.values(graph.invariants).flatMap(inv => inv.testIndexes || []);
  if(ownedIndexes.length !== manifest.tests.length) throw new Error(`IMPACT-7 test ownership count ${ownedIndexes.length} does not equal manifest count ${manifest.tests.length}.`);
  if(new Set(ownedIndexes).size !== ownedIndexes.length) throw new Error('IMPACT-7 deterministic test ownership contains duplicate manifest indexes.');
  ...
  if(missing.length || extra.length) throw new Error(`IMPACT-7 ownership mismatch. missingIndexes=${missing.join(',')} extraIndexes=${extra.join(',')}`);
  const exactProofIds = proofIds();
  if(new Set(exactProofIds).size !== exactProofIds.length) throw new Error('IMPACT-7 proof IDs must belong to exactly one bundle.');
